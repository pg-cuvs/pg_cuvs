...
/* ADR-075 Phase 1 (0.5.0): hardware-profile introspection.
...
-- ----------------------------------------------------------------
-- halfvec opclasses for pg_cuvs_hnsw (#162)
--
-- Mirrors pgvector's own halfvec_*_ops FOR TYPE halfvec USING hnsw exactly
-- (verified against a live pgvector 0.8.6 install), substituting only
-- `USING hnsw` -> `USING pg_cuvs_hnsw`, per the same rule that governs the
-- vector_*_ops block above: reference pgvector's own exported functions
-- directly, never reimplement them, because the delegated read-path scan
-- resolves them via index_getprocinfo on THIS opclass.
--
-- Structural note: unlike vector_*_ops above (FUNCTION 1[, 2] only), the
-- halfvec opclasses ALSO carry FUNCTION 3 (hnsw_halfvec_support) --
-- pgvector requires it for halfvec's hnsw opclasses specifically (float's
-- hnsw opclasses have no such slot upstream either). Omitting it here would
-- silently produce an opclass pgvector's own delegated AM routine cannot use.
--
--   CREATE INDEX my_idx ON items USING pg_cuvs_hnsw

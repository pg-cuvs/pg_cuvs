...
> **Current contract (2026-08-04):** extension version `0.7.0`,
...
| 3B | DiskANN/NVMe cold tier — **NO-GO**. 재검토 조건: 1B+ 수요, **AiSAQ 레이아웃 포팅 착수 판단**, 또는 128GB+ VRAM. 재개 시 구현 방향 ADR-072(2026-07-24 개정) — cold tier는 vchord 공존이 아니라 자체 소유이며, 남의 구현을 링크하지 않고 **AiSAQ 인라인 PQ 레이아웃을 자체 직렬화 계층으로 포팅**한다. **"cuVS PQFlash stable"은 트리거에서 제거됨** — 대상(MS DiskANN C++)이 `cpp_main`으로 동결되었고, 더 근본적으로 남의 릴리스 일정에 재개 시점을 묶는 형태였다. **(2026-08-04 보강)** 산술 분석 결과 그래프 경로는 **경제성으로 사망** — `초당 PQ 평가 = IOPS × R`이라 NVMe 1대가 GPU 수요를 0.13%로 잠그고 CPU 0.32코어가 포화시킨다. 따라서 재개 트리거에 **경제성 반증 K1 통과를 선행 조건으로 추가**(`fio` IOPS × R vs CPU 1코어, GPU·GDS 불요, 반나절). 잔존 경로는 RaBitQ+GDS(IVF)뿐이며 그것도 하드웨어 미확보. 상세 `docs/reports/2026-08-04-aisaq-gds-viability.md`, ADR-072/076 2026-08-04 추가 |
...
> **전략 재평가 (2026-06-07, ADR-061 / [design/strategy/workload-notes.md](design/strategy/workload-notes.md))**: 경쟁 데이터(VectorChord가 32억 벡터를 CPU+NVMe 월 $12k로 서빙)가 "규모=GPU" 전제를 깸. pg_cuvs 표적 = **쿼리당-비용 지배 세그먼트**(온라인 RAG, 멀티테넌트). 신규 1순위 후보 = **exact filtered brute-force**(D) — cuVS `cuvsBruteForceSearch` + BITSET prefilter 네이티브 지원 확인됨. 3O는 D의 post-filter를 BITSET pre-filter로 확장하는 순차 작업으로 재정의(대체 아님). 3P(아래)는 "규모 핵심" → "VRAM working-set 천장"으로 격하.
...
대상: `design/ops-gpu-playbook.md` · `docs/playbooks/` · (신규) `ARCHITECTURE.md` + 문서 맵 · (신규) `mkdocs.yml` + Pages 배포 워크플로
...
스펙: [design/specs/phase-record.md — Phase 3C, 3D](design/specs/phase-record.md) | ADR-013
...
스펙: [design/specs/phase-record.md — Phase 3N](design/specs/phase-record.md) | ADR-042
...
스펙: [design/specs/phase-record.md — fp16 입력 벡터](design/specs/phase-record.md) | ADR-054
...
**상태**: D 잔여로 이동. 스펙: [design/specs/phase-record.md — EXPLAIN GPU 타이밍](design/specs/phase-record.md) | ADR-055
...
스펙: [design/specs/phase-record.md — VACUUM tombstone 연동](design/specs/phase-record.md) | ADR-056
...
스펙: [design/specs/phase-record.md — Phase 4B](design/specs/phase-record.md) | ADR-035

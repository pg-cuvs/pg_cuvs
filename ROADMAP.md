...
| 3L | GPU brute force 검색 모드 (`.vectors` sidecar, `search_mode`/`bf_precision` GUC, micro-batching, sharded BF). installcheck 10/10, recall@10=1.0 (ADR-039) |
| 3M | 배치 검색 API (`CUVS_OP_SEARCH_BATCH`, `pg_cuvs_batch_search` SRF, CAGRA/BF/sharded 지원). installcheck 10/10, Q×K top-k 일치 (ADR-040) |
...
### Wave 4 — 빌드 성능 (프로파일링 → 최적화)

#### 4-preflight — 연산 지역성 프로파일링
**왜 먼저**: 4A/4B 최적화 착수 전에 빌드/검색/export 경로의 latency split을 실측해야 우선순위를 정량 근거로 잡을 수 있음. ADR-034/035/043의 근거가 코드 분석 기반 추정이므로 실측으로 검증/보정.

구현 항목:
- 4-pre-1: nsys daemon 프로파일링 (GPU 커널 vs IPC overhead 비율)
- 4-pre-2: perf backend 빌드 프로파일링 (heap scan cache miss 핫스팟, TOAST vs PLAIN 비교)
- 4-pre-3: pg_stat_io + perf probe buffer manager 측정 (page write overhead 정량화)
- 4-pre-4: 결과 문서화 (`docs/profiling-results.md`) + 4A/4B 우선순위 재검증

완료 기준:
- 검색/빌드/export 경로별 latency split 수치 기록
- TOAST vs PLAIN cache miss 수치가 ADR-043에 반영
- 4A/4B 우선순위가 실측 근거로 확정 또는 재조정

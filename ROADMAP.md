...
| **flat isolation 스펙 (flat 변종)** | isolation 3 스펙(delta_tombstone_snapshot/delta_interleaving/reindex_concurrent_delete)이 **`USING cagra`만** 검증 — A1은 동일 기계장치 상속이나 flat 직접 검증 안 됨. cagra 스펙을 `USING flat` 변종으로 복제 추가 (ADR-074) | 순차 |
| **B 최종 거취 결정** | 제거 vs experimental 강등. (선택) STORAGE PLAIN 실측·B 최적화(pinned-H2D/RMM-pool/마샬링 복사 제거) 측정 후 (ADR-074) | 순차 |
| **filtered 교차점 + 런타임 적응 라우팅** | live `auto`의 전제: filtered wedge(B의 filter-first vs CPU exact-filtered) 교차점 측정 + 동시성/offload 런타임 신호 + **하드웨어-포터블 cost**(VM 스펙 고정 금지, 배포 장비별 계수 선택) (ADR-074/ADR-069) | 트리거 |

...
| Legacy dataset | Cohere `wikipedia-2023-11-embed-multilingual-v3` EN, 1M × 1024 (cosine, L2-normalized) — secondary, superseded as primary by wiki_all_1M; see archive [§2.1a](docs/experiments/benchmark-archive.md#21a-re-measured-via-cuvs-bench-nvidias-tool-ext-050-2026-07-16--legacy-dataset-cohere) |
...
### 2.1 Real embeddings — Cohere Wikipedia 1M × 1024 (cosine) [legacy dataset] — moved

The Cohere lineage — §2.1 (legacy anbench run, with its k=100 and `index_bytes = 0`
defects annotated) and §2.1a (the cuvs-bench re-measurement, ext 0.5.0, 2026-07-16) —
moved verbatim to
[`docs/experiments/benchmark-archive.md`](docs/experiments/benchmark-archive.md#21-real-embeddings--cohere-wikipedia-1m--1024-cosine-legacy-dataset).
The project's canonical real-embedding dataset is **wiki_all_1M**, so the current
real-embedding results are §2.1b (cross-machine reproduction) and §2.1c (#98 four-algorithm
sweep) below.

### 2.1b Cross-machine reproduction (wiki_all_1M, RunPod vs Brev) — canonical, why only *ratios* are portable

The [§2.1a](docs/experiments/benchmark-archive.md#21a-re-measured-via-cuvs-bench-nvidias-tool-ext-050-2026-07-16--legacy-dataset-cohere) sweep was re-run through the same cuvs-bench Postgres backend on
...
ordering is the one [§2.1a](docs/experiments/benchmark-archive.md#21a-re-measured-via-cuvs-bench-nvidias-tool-ext-050-2026-07-16--legacy-dataset-cohere) already described from the other direction: `pgcuvs_hnsw_import`
...
- **Most pilot/crossover data is synthetic.** Clustered synthetic flatters IVF
  ...
  canonical real-embedding anchor, with [§2.1a](docs/experiments/benchmark-archive.md#21a-re-measured-via-cuvs-bench-nvidias-tool-ext-050-2026-07-16--legacy-dataset-cohere) (Cohere) as a secondary one; treat
...
| Real Cohere 1M×1024 (archive [§2.1a](docs/experiments/benchmark-archive.md#21a-re-measured-via-cuvs-bench-nvidias-tool-ext-050-2026-07-16--legacy-dataset-cohere)) | `run_pg_cuvsbench.py … --dataset cohere` |
| Real Cohere 1M×1024 (archive [§2.1](docs/experiments/benchmark-archive.md#21-real-embeddings--cohere-wikipedia-1m--1024-cosine-legacy-dataset), legacy anbench) | `bench/legacy/run_cohere.sh --n 1000000 --gpu 0` |
...
## Appendix A — Legacy anbench Cohere run (2026-06, superseded) — moved

Moved verbatim to
[`docs/experiments/benchmark-archive.md`](docs/experiments/benchmark-archive.md#appendix-a--legacy-anbench-cohere-run-2026-06-superseded).
It is the original `bench/legacy/run_cohere.sh` output on Cohere 1M × 1024, with its two
code-era defects (k=100 fixed, `index_bytes = 0`) annotated there.
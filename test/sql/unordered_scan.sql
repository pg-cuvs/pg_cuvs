...
RESET enable_seqscan;
-- Routing check in ivfpq_smoke.sql's exact shape: explicit ::vector cast, no
-- enable_seqscan forcing. That is the form proven to reach the ivfpq AM under
-- the Tier-1 shim.
SELECT us_uses_index('SELECT id FROM us_ivfpq_tbl ORDER BY embedding <-> ''[1,0,0,0]''::vector LIMIT 1', 'us_ivfpq_idx')

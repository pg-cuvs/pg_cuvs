...
RESET enable_seqscan;
-- ============================================================ ivfpq
-- #149 left a NOTE here saying these assertions were probably vacuous because
-- the planner never considered this ivfpq index, cause undiagnosed. #151 found
-- the cause: CuvsIvfPqOptions violated the reloptions layout contract, so every
-- WITH-clause ivfpq index resolved a garbage artifact directory and lost the
-- no_artifact gate at 1e15 -- on real GPUs too, not just the shim. With that
-- fixed the assertions below are real, and pattern 4 is restored for ivfpq.
...
SELECT us_uses_index('SELECT id FROM us_ivfpq_tbl ORDER BY embedding <-> ''[1,0,0,0]'' LIMIT 5', 'us_ivfpq_idx')

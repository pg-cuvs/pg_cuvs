...
-- ============================================================================
-- Step 2: cuvs.k reflected in requested_k, and metric derived from opclass.
-- ============================================================================
-- cuvs.k bounds the GPU top-k (SQL LIMIT is applied by the executor on top).
-- A search after SET cuvs.k=7 must report requested_k=7 (deterministic).
SET cuvs.k = 7;
SET enable_seqscan = off;
SELECT count(*) AS k7_rows
FROM (SELECT id FROM ec ORDER BY embedding <-> '[1,0,0,0]'::vector LIMIT 5) s;
RESET enable_seqscan;
SELECT requested_k FROM pg_stat_gpu_search WHERE index_oid = 'ec_cagra'::regclass;
RESET cuvs.k;
-- metric column is derived from the index opclass (l2/cosine/ip), not always L2.
SELECT index_name, metric FROM pg_stat_gpu_search
WHERE index_oid IN ('ec_cagra'::regclass, 'ec_cos_cagra'::regclass, 'ec_ip_cagra'::regclass)
ORDER BY index_name;
...
RESET enable_seqscan;
-- DELETE + VACUUM exercises the ambulkdelete tombstone path (Phase 3A-4).
-- Small deletes use tombstones and do NOT mark the index stale.
DELETE FROM ec WHERE id = 99;
VACUUM ec;
SELECT stale FROM pg_stat_gpu_search WHERE index_oid = 'ec_cagra'::regclass;

-- ============================================================================
-- Step 5: pending-delta merge for cosine + IP metrics (Phase 3A).
-- The CPU delta distance must match the daemon's cuVS scale per metric so the
-- merge orders correctly: cosine = 1 - cossim (ascending), IP = raw dot
-- (descending). Probe a vector whose new delta row is the unique extremum, so
-- the merge must surface it first regardless of CAGRA approximation on the base.
-- ============================================================================
-- cosine: id 99 equals the probe direction -> cosine distance 0 -> nearest.
INSERT INTO ec_cos VALUES (99, '[0.5,0.5,0,0]');
SET enable_seqscan = off;
SELECT id FROM ec_cos ORDER BY embedding <=> '[0.5,0.5,0,0]'::vector LIMIT 1;
RESET enable_seqscan;
-- ip: id 99 has the unique largest inner product with the probe -> nearest.
INSERT INTO ec_ip VALUES (99, '[2,2,0,0]');
SET enable_seqscan = off;
SELECT id FROM ec_ip ORDER BY embedding <#> '[1,1,0,0]'::vector LIMIT 1;
RESET enable_seqscan;

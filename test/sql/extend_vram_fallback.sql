...
-- ----------------------------------------------------------------
-- Test 1: EXTEND 실패 확인 (budget 차단으로 extend_count=0 유지)
-- ----------------------------------------------------------------
...
-- 검색이 id=55를 반환하면 delta fallback이 동작했음을 증명한다.
-- (delta_rows는 daemon의 e->delta_rows 캐시로, COMPACT/로드 시에만
-- 갱신되므로 backend delta write 직후 STATUS 체크에서는 0으로 보인다.)
-- ----------------------------------------------------------------

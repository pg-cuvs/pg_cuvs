...
| **IVF-PQ engine (axis-wide)** | 🟢 smoke-validated, sweep needs build knobs | `ivfpq` AM done (ADR-049); `forced-ivfpq` wired (`b858656`) and **end-to-end smoke PASS on A100** (run #30, N=100k·dim=1024): plan guard OK, `build_kind=pq`, **resident VRAM 54 MB vs 410 MB raw f32 = 7.6× compression**, build 5.4s | recall@10 **caps at 0.937** even at n_probes=512 (= all lists) → PQ-quantization floor, not search breadth. **iso-recall=0.95 unreachable via n_probes alone** — needs build-knob sweep (`pq_dim`/`pq_bits`), see harness gap below |
...
- **`pq_dim`/`pq_bits` as cell/build knobs (NEW gap, found by run #30)** — the recall knob (`n_probes`) is query-time only; at dim=1024 ivfpq recall caps at **0.937 < 0.95** because the loss is PQ quantization, not search breadth (n_probes=512 already probes all √N≈316 lists). Protocol §163 wants ivfpq to self-tune `n_lists·pq_bits·n_probes`, but `runner.py` fixes the build params per cell. To hit iso-recall=0.95 the harness must sweep **build-time** `pq_dim`/`pq_bits` (rebuild per point — more expensive than the query-time sweep), or accept ivfpq as a *below-0.95* high-compression operating point and report it on its own recall row (exact paths already get their own row; ivfpq's natural recall ceiling is a finding, not a failure).

...
| **IVF-PQ engine (axis-wide)** | 🟡 harness wired, UNMEASURED | `ivfpq` AM done (ADR-049); **protocol v3 lists it first-class** in the config axis (`BENCHMARK_PROTOCOL.md` §155) + self-tuning knobs `n_lists/pq_bits/n_probes` (§163) + a VRAM-budget cell as a 1급 D1/D6 axis (§84); **`forced-ivfpq` now in `runner.py`** (commit `b858656`) | run it: VRAM-budget cell (ivfpq vs cagra at fixed VRAM) + 50M arm. No more code blocker — needs GPU time + plan-guard confirmation on the VM |
...
- **`forced-ivfpq` config in `runner.py`** — ✅ **DONE** (build `USING ivfpq` with `n_lists`=√N + AM-default `pq_bits`/`pq_dim`; recall knob `cuvs.ivfpq_n_probes` sweep [16..512]; plan guard; `engines/forced-ivfpq.sh` wrapper; selfcheck green). Full GPU build/query is VM-only — **first action on the VM is to run one ivfpq cell and confirm the plan guard passes** before trusting any row. Unblocks the 50M IVF-PQ arm (D6) and the D1/D6 VRAM-budget cell.

...
| **Stage D** (filter/incremental/Pareto/concurrency/storage) | **measured items DONE 2026-06-16** (autonomous push, runs #30–#44): D2 filter ✅, D3 incremental-v1 ✅, D4 concurrency ✅, D8 storage ✅, dim/auto ✅, D6 cite-only ✅. Remaining: D1 $-Pareto (price input main-blocked → `d1_pareto.py` workaround) + D3 v2 scenarios (FIFO/upsert/recall-drift/concurrent). See §5. |
...
- **D1 Pareto** — partial. **VRAM-budget cell**: measured — ivfpq 54 MB@0.937 / 64 MB@0.965 (pq_dim 512/1024, run #30/#31) vs cagra/flat resident ~410 MB raw → ivfpq bends the resource axis 6–7×. **$-Pareto remaining**: the runner's cost fields are empty (no `--price-usd-hr` passed; the `instance_type`/`price` bench.yml inputs don't exist → main-branch blocked). Workaround: a post-hoc aggregator applying a known A100 $/hr to the existing `qps`/`p99` rows → `$/1M`, `$/QPS@p99`. iso-$ CPU arm still needs a CPU-instance dispatch (separate infra).

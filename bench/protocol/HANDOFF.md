...
| **D3 incremental** | ✅ **DONE** | `runner_incremental.py`, 5 scenarios + PK(id) (runs #43–#54). **append** flat 431 / no-index 1573; **fifo** 345 / 802; **upsert** 417 / 1483 ops/s → no-index (W1) beats flat (W2) 2.3–3.6× on every write (ADR-074). **recall-drift** after 2000 fifo ins+del: flat **recall@10=1.0, 0 leaked** (streaming index stays exact). **concurrent** (query QPS under ingest): no-index 1.5 qps (seqscan-bound), 0% degradation; **flat concurrent ingest+query errors** → finding: the single daemon doesn't cleanly serve simultaneous extend+search from two sessions (consistent with ADR-074 read-heavy→flat / write-heavy→no-index) | follow-up: diagnose the flat concurrent extend+search path (possible daemon-concurrency limitation) |
...
- **D3** — ✅ **DONE** (`runner_incremental.py`, 5 scenarios via `PGCUVS_INC_SCENARIO` + PK(id), runs #43–#54). Clean throughput: append 431/1573, fifo 345/802, upsert 417/1483 ops/s (flat/no-index) — no-index (W1) beats flat (W2) 2.3–3.6× everywhere. recall-drift: flat **recall@10=1.0, 0 leaked** after 2000 fifo ins+del (streaming index exact). concurrent: no-index 1.5 qps (seqscan-bound, 0% degr); **flat concurrent ingest+query errors** — the single daemon doesn't serve simultaneous extend+search from two sessions (a real finding; matches read-heavy→flat / write-heavy→no-index). **Follow-up**: diagnose the flat concurrent path (likely a daemon-concurrency limitation, separate investigation).

...
- **`forced-ivfpq` config in `runner.py`** — the harness knows only forced-hnsw/forced-cuvs/forced-flat; the `ivfpq` AM (ADR-049) is in the product but not benchmarkable here. Add the build (`USING ivfpq` + reloptions `n_lists`/`pq_bits`/`pq_dim`) + search (`cuvs.ivfpq_n_probes` sweep) path. Unblocks the 50M IVF-PQ arm (D6) and any VRAM-constrained large-N cell.

...
| **D3 incremental** | ✅ **DONE** | `runner_incremental.py`, 3 scenarios + PK(id) + recall-drift (runs #43–#52, clean). **append** flat 431 / no-index 1573; **fifo** 345 / 802; **upsert** 417 / 1483 ops/s → no-index (W1) beats flat (W2) 2.3–3.6× on every write workload (ADR-074). **recall-drift after 2000 fifo ins+del: flat recall@10 = 1.0, 0 leaked** — streaming index stays exact (inserts searchable, deletes excluded) | follow-up: concurrent-query-during-ingest (query QPS degradation under ingest; needs threading) |
...
- **D3** — ✅ **DONE** (`runner_incremental.py`, 3 scenarios via `PGCUVS_INC_SCENARIO`, runs #43/#46/#47): **append** flat 431 / no-index 1573 rows/s; **fifo** (INSERT head + DELETE tail) 80 / 109 ops/s; **upsert** (INSERT-new / UPDATE-existing) 143 / 192 ops/s. no-index (W1) beats flat (W2) on every write workload — ADR-074 confirmed; flat's streaming insert/delete/update all functional. **Follow-ups**: add `PRIMARY KEY(id)` so delete/update aren't seqscan-bound (cleaner absolutes); recall-drift (window GT recompute) + concurrent-query-during-ingest (needs threading).

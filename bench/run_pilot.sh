...
if [ "$ENGINE" = "hnsw" ]; then
  ...
  set_param() { echo "-c cuvs.k=$1 -c cuvs.index_dir=$IDX_DIR"; }
else
...
if [ ! -s "$OUT" ]; then
  echo "system,index,N,dim,k,recall_target,build_s,qps,p50_us,p95_us,p99_us,avg_us,recall_at_k,params" > "$OUT"
fi
echo "$ENGINE,$AM,$N,$DIM,$K,$RECALL_TARGET,$BUILD_S,${QPS:-NA},$P50,$P95,$P99,$AVG,$CHOSEN_RECALL,param=$CHOSEN" >> "$OUT"

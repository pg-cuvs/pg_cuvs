...
| `cagra_dynbatch_160.csv` | 2026-08-05 | `bench/cuvs_bench_backend/cagra_dynbatch_sweep.py --db shadeform` (ext 0.7.0) | wiki_all_1M, 1M×768 (`t_cagra`, gd=32/igd=128/`build_algo=ivf_pq`) | `massedcompute_A100_sxm4_80G` (Brev `pg-cuvs-verify-3fix-b`) — **not the DGX variant** behind `pg_cuvsbench_98.csv` | **feature validation sweep** — the #160 `cuvs.cagra_batch_wait_us` (N × window) grid, not a canonical two-axis run. Cite ratios within this file only. See [`BENCHMARK.md` §2.1d](../../BENCHMARK.md) |

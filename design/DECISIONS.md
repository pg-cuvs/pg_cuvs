...
**구현 발견 (pre-existing, 마이크로배칭과 무관)**: planner는 GPU 인덱스 cost가 seqscan보다 낮을 때만 cagra 인덱스로 라우팅한다. 별도 단발 연결의 GPU 쿼리는 seqscan으로 fallback할 수 있고(이는 CAGRA에도 동일하게 적용되는 기존 cost 동작), 그 경우 데몬에 도달하지 않아 마이크로배칭이 engage하지 않는다. 따라서 동시 coalescing의 throughput 이득은 주로 **warm long-lived 세션**에서 실현된다. 단일 세션 GPU 경로(installcheck)에서는 정상 동작. 동시 연결의 seqscan fallback 자체는 3L 범위 밖의 별도 cost-model 조사 대상.
...
|-----------|-----------------|-------|------|
| (a) CAGRA 빌드 시간 | 측정 예정 | 측정 예정 | `CREATE INDEX USING cagra` wall-clock |
| (b) heap 크기 | 측정 예정 | 측정 예정 | `pg_relation_size(rel)` + TOAST 포함 |
| (c) 검색 latency (CAGRA GPU) | 측정 예정 | 측정 예정 | `EXPLAIN (ANALYZE)` p50. storage 차이가 검색에 영향을 주지 않아야 함(검색은 CAGRA 그래프 경유이지 heap scan이 아님) |
| (d) 검색 latency (pgvector HNSW CPU) | 측정 예정 | 측정 예정 | heap recheck 시 detoast 비용 차이 확인 |
| (e) INSERT throughput | 측정 예정 | 측정 예정 | `COPY` 또는 batch INSERT. PLAIN에서 tuple이 커지면 WAL volume + page split이 영향을 줄 수 있음 |

완료 후 이 테이블의 "측정 예정"을 실측 수치로 갱신하고, `docs/best-practices.md`의 "PLAIN storage의 효과" 섹션에 실측 결과를 포함한다. 빌드 시간 차이가 기대(~25-35%)보다 작으면 NOTICE의 문구와 권장 강도를 재조정한다.

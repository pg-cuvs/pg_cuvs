...
| 0.0001 | (미측정) → **1.000** | 10.00 | — → 35.16 ms |
   ...
   D-wedge는 exact BF + 후처리이므로 깊이만 충분하면 **정의상 exact**다. 코퍼스 크기로 클램프되므로 극단 선택도에서는 전체 top-k로 수렴하고, brute force는 어차피 모든 거리를 계산하므로 선택 깊이 증가 비용만 든다.

3. **두 경로의 거래 구조가 뒤바뀌었다.** #80 이후 D-wedge는 전 구간 exact이고 3O는 어디서도 더 정확하지 않다. 3O가 더 빠른 구간은 (a) 차이가 5 ms 미만이거나(sel 0.001~0.01), (b) recall이 무너진 구간(sel < 0.001)뿐이다. 즉 **3O가 우세한 선택도 구간이 존재하지 않는다.**

| | 3O | D-wedge (#80 이후) |
|---|---|---|
| recall | sel<0.001에서 붕괴 (0.86→0.27) | 전 구간 1.000 |
...
즉 초판의 "상관이 더 가혹하다"도, 2판의 "무상관이 최악이다"도 틀렸다. 무상관은 양의 상관보다 나쁘지만 최악은 아니며, **반상관이 최악**이다. 이 조건에서 고정·비례 어느 오버페치도 exact를 보장하지 못하므로, #86은 프리픽스가 k를 못 채우면 전수로 재검색해 exactness를 실제로 보장하도록 고쳤다(반상관 필터 2000행 실측에서 기대값과 10/10 일치).

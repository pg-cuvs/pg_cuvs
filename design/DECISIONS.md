...
**결정**: cuVS CAGRA pre-filter(bitvector mask)를 활용한다. PG executor가 filter 조건을 비트맵으로 평가해 shm으로 daemon에 전달하면, daemon은 마스크된 노드만 탐색하는 cuVS filtered search를 호출한다.

**구현 방향**:
- `CuvsCmdFrame`에 `filter_shm_key[64]` 추가 (비트맵 shm 이름; 빈 문자열 = unfiltered).
- backend: filter 조건을 heap TID 비트맵으로 평가 → shm 세그먼트에 기록.
- daemon: cuVS filtered search API 호출 (마스크 노드 스킵).
- fallback: 비트맵 shm 생성 실패 시 기존 unfiltered + post-filter 경로 + WARNING.
- GUC `cuvs.prefilter_threshold`(비트맵 밀도 상한; 이 이하면 pre-filter, 초과이면 post-filter).

**대안 기각**:
- post-filter + K 과다 요청: 선택성이 높을수록 K 추정이 불안정하고 GPU 낭비 누적.
- 항상 pre-filter: 저선택성 필터에서 cuVS graph traverse 품질 저하.

**트레이드오프**: 고선택성 필터에서 GPU 후보 품질·처리량 대폭 향상. 비트맵 생성·shm 전달 오버헤드 (~수백 μs), IPC 프레임 확장.
...
**문제**: PG의 `statement_timeout`과 query cancel(`pg_cancel_backend`)이 daemon IPC로 전파되지 않는다. GPU 검색이 응답 없이 걸리면 backend는 `statement_timeout` 이후에도 UDS recv에서 무기한 대기한다. 이는 운영 환경에서 연결 고갈 및 서비스 장애로 이어질 수 있는 구조적 공백이다.

**결정**: backend의 취소 시그널이 daemon에 전파되도록 두 계층에서 처리한다.

**backend 측**:
- UDS recv에 `SO_RCVTIMEO` 또는 `poll(fd, timeout)` 루프 적용.
- 루프 내 `CHECK_FOR_INTERRUPTS()` 호출로 PG cancel signal 감지.
- 취소 감지 시 daemon에 `CUVS_OP_CANCEL` 전송 후 오류 반환.

**daemon 측**:
- 검색 loop에서 cancel op 수신 시 진행 중인 cuVS 검색 중단 (cuVS API cancel 지원 여부에 따라 thread join + timeout 방식 fallback).
- cancel 응답 후 해당 연결 상태 초기화.

**GUC**: `cuvs.ipc_timeout_ms`(기본 5000ms) — `SO_RCVTIMEO` 상한 설정.

**트레이드오프**:
- cuVS 검색 자체가 non-cancellable이면 daemon이 검색 완료까지 대기 후 응답. backend는 이미 취소됐으므로 결과 폐기. VRAM 낭비는 있으나 연결 고갈은 방지.
- `ipc_timeout_ms` 너무 짧으면 정상 대형 검색에서 false cancel. 기본값은 보수적으로 설정.
...
## ADR-057 — Subtransaction-aware DROP 수집 (RegisterSubXactCallback)

**날짜**: 2026-06-07
**상태**: 트리거 대기 — 의도적 보류 (ADR-023 commit-only 단순화의 알려진 한계)

**문제**: `SAVEPOINT s; DROP INDEX; ROLLBACK TO s; COMMIT;` 패턴에서 `object_access_hook`이 OAT_DROP을 `cuvs_pending_drops`에 수집할 때 어느 subtransaction에서 발생했는지 기록하지 않는다. SAVEPOINT가 롤백돼도 OID가 리스트에 남아 상위 트랜잭션 commit 시 `cuvs_ipc_drop`이 발사된다 — 살아있는 인덱스의 GPU artifact가 조기 삭제되고 REINDEX로만 복구 가능하다.

참고로, `BEGIN; DROP INDEX;` 후 커밋하지 않으면 daemon에 아무것도 전송되지 않는다(`XACT_EVENT_COMMIT`에서만 발사). 이 케이스는 안전하다.

**결정(조건부)**: `RegisterSubXactCallback`으로 `SUBXACT_EVENT_ABORT_SUB` 이벤트를 수신해 롤백된 subxact에서 수집된 OID를 `cuvs_pending_drops`에서 제거한다.

**구현 방향**:
- `cuvs_pending_drops` 각 엔트리에 `SubTransactionId`(또는 nesting depth) 태깅.
- `RegisterSubXactCallback` 등록: `SUBXACT_EVENT_ABORT_SUB` 수신 시 해당 subxact level 이상에서 수집된 OID 제거.
- 상위 트랜잭션 `XACT_EVENT_COMMIT`에서는 남은 OID만 통지.

**보류 이유**:
- ADR-023이 의도적으로 택한 "commit-only 단순화" (`cuvs_pending_drops` = 단순 OID 배열)를 되돌리는 셈 — 구조 변경 + 콜백 관리 복잡도 증가.
- 실제로 `SAVEPOINT ... DROP ... ROLLBACK TO ... COMMIT` 패턴 자체가 매우 드묾.
- 발생해도 정합성 손실이 아닌 artifact 조기 삭제(살아있는 인덱스가 stale 처리) → REINDEX로 완전 복구.

**트리거**: SAVEPOINT+DROP 패턴에서 실측 artifact 손실이 프로덕션 문제로 제기되는 시점.

...
**트리거 개정** — ADR-026의 3B 트리거 중 **"cuVS PQFlash stable"을 제거**한다. 두 번
틀렸다: 대상이 동결된 것이 첫째이고, 더 근본적으로 **남의 릴리스 일정에 우리 재개 시점을
묶은 형태**라는 것이 둘째다. 대체 트리거는 **"AiSAQ 레이아웃 포팅 착수 판단"** — 우리가
결정할 수 있는 것이어야 한다는 원칙을 만족한다. 나머지 둘(1B+ 수요, 128GB+ VRAM)은 유지.

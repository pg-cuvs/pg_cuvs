...
**결론**:
1. **3O에서 VecFlow형 recall 상한·붕괴는 재현되지 않았다.** 선택도 0.001까지 recall 0.983~0.996이고 반환 행 수는 항상 k=10을 채운다. ADR-079가 제기한 리스크는 **우리 구현·이 데이터셋 범위에서는 기각**된다. 3O를 "제한적 근사 확장"으로 강등할 근거가 측정으로는 나오지 않았다.
2. **붕괴하는 쪽은 D-wedge다.** 선택도 0.05 이하에서 recall이 0.78 → 0.16 → 0.08 → 0.01로 무너지고, 반환 행 수가 0.11개까지 떨어져 **k조차 채우지 못한다**. 원인은 코드에 그대로 있다: `src/pg_cuvs.c`의 `k_fetch = Min(k*4, 4000)` — **선택도와 무관한 고정 4× 오버페치** 후 후처리 필터. 선택도 s에서 기대 생존 수 ≈ 4k·s이고 s=0.001이면 0.04로, 실측 0.11과 자릿수가 맞는다.
3. **라우팅 임계 T의 실측 검증**: 게이트는 `sel < cuvs.filter_auto_threshold`(기본 0.05)일 때만 3O로 보낸다. 즉 **sel = 0.05 자체는 D-wedge가 처리하는데 그 지점에서 이미 recall 0.782**다. 무릎은 0.05와 0.1 사이에 있다(0.1에서는 D-wedge도 0.999). ADR-079의 "T≈1-2K 후보" 추정은 절대 후보 수로는 같은 자릿수지만(0.05×1M = 50K는 과하고, 0.001×1M = 1K가 붕괴 심부), **비율 임계로 표현할 때 0.05는 낮게 잡혀 있다.**
4. **지연은 반대 방향으로 교차한다.** 고선택도에서 3O는 D-wedge보다 느리다(0.5에서 38.8 ms vs 13.8 ms — 50만 TID의 BITSET 구성 비용). 즉 임계는 recall과 지연 양쪽을 동시에 거래하는 지점이고, 임계를 무한정 올릴 수는 없다.

**도출 액션**:
1. `cuvs.filter_auto_threshold` 기본값을 **0.05 → 0.1로 상향** 검토 — 0.1에서는 D-wedge도 0.999를 내고 3O보다 2배 빠르다. (별도 이슈)
2. 더 근본적으로 **D-wedge 오버페치를 선택도에 비례**시킨다: `k_fetch = clamp(k/sel · 안전계수, k*4, 4000)`. 고정 4×가 붕괴의 직접 원인이다. (별도 이슈)
3. 다른 데이터셋·차원에서 3O 곡선 재확인 — 본 측정은 wiki_all_1M(768d) 단일 데이터셋이다.
...
**관련**: ADR-079(리스크 제기), ADR-048(3O), ADR-063(D-wedge 라우팅), 이슈 #76. 산출물: `bench/adr079_3o_recall.py`, `bench/results/adr079_3o_recall.csv`.

...
>
> 관계:
> - [`positioning.md`](positioning.md) = *엔지니어링 정합성* 포지셔닝(source-of-truth/MVCC/fail-closed). **이 노트와 충돌점 있음** (아래 쟁점 E).
...
`positioning.md`가 "exact GPU vector search for PostgreSQL"를 **Avoid 메시징**으로,
...
**교정 (positioning.md, 2026-06-07 반영)**:
...
**핵심 (범위 한정)**: **≤64 활성 파티션**에서는 새 아키텍처 0줄 — 기존 3D cold registry + LRU eviction(`pg_cuvs_server.c`)이 그대로 온라인-스케일 캐시로 동작. 단 `MAX_INDEXES=64`가 그 너머에선 **하드월**(아래 Item 2A).

### Item 1 — 정식화 완료 (#32)
`test/sql/partition_prune.sql` + golden(결정적: pruned=단일 child Index Scan, confinement, IN(2,3)=2파티션, 무필터=fan-out) + `infra/scripts/recipes/tenant-partition-recipe.sql`. REGRESS 18/18 GREEN.

### Item 2 — 스케일 스파이크 (2026-06-08, 측정)
**2A. `MAX_INDEXES=64`는 하드월**: 130 tenant 파티션 빌드 → **64만 resident**(LRU=last-built tenant 67–130), 그리고 이건 **슬롯 캡이지 VRAM 아님**(6MB/38GB 사용). 축출된 파티션(tenant 1) 쿼리 → **ERROR** `cagra index not loaded ... breaker open ... REINDEX` + CPU fallback. **런타임 슬롯-캡 축출은 auto-reload 안 됨**(startup/budget 축출의 cold-reload와 다름). `REINDEX`로 복구(다른 LRU 축출). cold registry도 `MAX_INDEXES=64`. → **수백+ 테넌트는 현재 코드로 매끄럽게 미지원**(정확한 64 vs 128 경계는 미특성화).
**2B. 현실크기 캐시-미스 꼬리**: 122MB 인덱스(80k×384)에서 cold reload **≈750ms** vs hot **≈1.5ms**. reload 처리량 ≈ ~150MB/s(1.5MB→15ms와 일관) → **GB급 파티션은 수 초**. locality로 amortize, **north-star(빠른 H2D 로드)가 직접 깎는 꼬리**.

**남은 갭 → 백로그**: `MAX_INDEXES` 상향/동적화 + 런타임-축출 인덱스의 auto-reload 배선(현재 REINDEX 필요). 이게 "수천 테넌트 온라인"의 선결 조건. PG planning 오버헤드 @ 많은 파티션도 미측정.
...
3. **검증 완료(이번 세션)**: ~~E~~ 해소(positioning 교정), D cuVS API 검증, **G 온라인-스케일(파티션 pruning + LRU) 스파이크 검증**.

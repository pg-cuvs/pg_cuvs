...
- `pg_stat_gpu_search`는 Phase 2 Step 1에서 구현됐다. 아직 `requested_k`/`returned_k`, fallback reason, GPU kernel/IPC/CPU recheck split, cache/stale counters는 후속 단계 계측이 필요하다.
...
이 절의 Phase 2는 제품 로드맵 단계다. 위 후속 hardening의 "Phase 2(edge_cases/scan fixes)"는 이미 완료됐고, Product Phase 2 Step 1 `pg_stat_gpu_search`도 완료됐다. 남은 주요 작업은 LIMIT-k, metric identity, cost model, write/staleness, large-build memory 모델, tiered cache 계측이다.
...
상태: 완료. `2542c8e`와 `a8e46d6`로 구현/검증됐다.

구현된 계약:
- stats source of truth는 daemon memory의 `IndexEntry`다.
- `handle_search`가 `g_index_mutex` 아래에서 per-index search/error count, latency sum, log2 latency histogram, last status/error를 갱신한다.
- IPC `CUVS_OP_STATUS`가 daemon stats를 반환한다.
- materialize-mode SRF `pg_cuvs_gpu_search_stats()`가 SQL view `pg_stat_gpu_search`를 backing한다.
- daemon down/unavailable은 PostgreSQL ERROR가 아니라 empty result set으로 처리한다. monitoring이 `pg_stat_*`류 view를 계속 poll할 수 있게 하기 위한 정책이다.
- dropped index OID는 `index_name NULL`로 렌더링하고 backend crash를 피한다.
- latency percentile은 daemon-side에서 `cuvs_lat_percentile` / `cuvs_lat_bucket_index` helper로 approximate p50/p95/p99를 계산한다.
...
검증 완료:
- unit 96 passed. percentile helper의 empty/single/uniform/skewed/clamp case 포함.
- regression 3/3. `edge_cases`에서 deterministic facts(count, identity, `p99 >= p50`)만 assert.
- integration scenario 8. `search_count` increment, row identity, daemon-down empty result + backend alive 확인.
- e2e PASS. l2/cosine/ip index가 restart across stable.

의도적 deferral:
- `gpu_kernel_us` / `ipc_us` / `cpu_recheck_us` split은 아직 없다. 현재 daemon은 wall-clock latency 하나만 가진다. Step 2/3 instrumentation에서 추가한다.
- `requested_k`, `returned_k`, fallback reason은 LIMIT-k와 fallback/cost workstream에서 추가한다.
- counters는 daemon memory resident이며 rebuild/reload/restart에서 reset된다. PostgreSQL native `pgstat` integration은 MVP 이후 검토한다.

...
- **무회귀 게이트**: `cuvs.enable_phys_cost`(기본 on) ∧ 프로파일 로드 성공 ∧ `(probe_status & 필요비트)==필요비트`일 때만 물리; 아니면 레거시 상수 **바이트 동일**(Tier-1 shim probe_status=0 / 미프로브 / GUC off). dim은 plan-time syscache typmod(`table_open` 없음). 방어: κ NaN/Inf/≤0 거부, hbm_bw 타당범위 클램프 + isfinite 가드, probe try/catch(데몬 abort 방지).

...
> **측정 단위 = 배포(§0).** flat·cagra·ivfpq는 컬럼당 하나만 빌드하는 빌드-타임 선택이므로,
> 플래너의 per-query 결정은 `seqscan ↔ {빌드한 그 인덱스}`(+무인덱스 transient-B)다. 따라서
> Stage B는 **배포별로 따로** 측정한다 — cagra 테이블에서 `seqscan↔cagra`, flat 테이블에서
> `seqscan↔flat`. **flat↔cagra를 한 셀에서 5지선다로 비교하지 않는다**(둘 다 빌드는 비현실적).

### 6.1 루프
```
EXPLAIN 스윕 (배포별: cagra 테이블 / flat 테이블, 실행 없음) — 물리 공식의 seqscan↔엔진 선택 덤프
...
**배포(cagra/flat)별 · physics vs legacy 레짐별**로, 그 배포의 두 경로(seqscan vs 빌드한 인덱스)
추정 덤프 + **물리 항 분해** + `pg_cuvs_hw_profile()` 스냅샷:
```
...
- 보정 보고서 `docs/experiments/cost-model-calibration.md`: 물리 공식, before/after 라우팅, regret 표, ε-밴드 좌표, 판별 플립, DEFAULT 폴백 증거, 도달 불가 구간(§13).
...
| N | GT | 비고 |
|---|----|------|
| ≤ 1M | exact BF(numpy/faiss-cpu flat 또는 **flat AM 자체**) | 기존 `bench/legacy/gt.py` |
...
| 보정 보고서 | `docs/experiments/cost-model-calibration.md` |

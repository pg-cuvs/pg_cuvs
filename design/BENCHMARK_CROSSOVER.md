...
- `bench/legacy/gt.py` — exact brute-force GT (numpy/GPU).
...
**데이터**: `bench/legacy/test_3i_bench.py`의 `random.Random(42)` 기반 synthetic random 벡터.
...
- restart-after-import: 재시작 전후 결과 동일 확인 (`bench/legacy/test_3i_restart.sh`)
...
### 14.2 MIG(Multi-Instance GPU) 기능 검증 (`bench/legacy/test_mig.sh`)
...
(L2-normalized → L2 거리 = cosine 거리, `bench/legacy/run_cohere.sh` 실행)

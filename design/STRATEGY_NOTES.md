...
> 상태: **탐색 노트 (미결정).** 결정이 굳으면 ADR로 승격 → DECISIONS.md.
...
## D. exact filtered brute-force wedge (검증 필요)
...
4. **cuVS API 검증 선행**: 26.04 brute-force/CAGRA의 bitset/bitmap prefilter 시그니처 확정 (`cuvs::core::bitset` + brute_force search filter로 추정, build_algo처럼 헤더 확인 필요).
...
| **3A/3Q (delta/streaming)** | 기능 | 숨은 정체성 = GPU에 없는 freshness/spill 계층 인위 구축. 근본 약점(B) 보완이라 가치 유지 |
...
2. **다음 한 수 (택1)**:
   - (a) **cuVS API 검증 스파이크**: VM 띄워 26.04 헤더/예제로 brute-force + bitset prefilter 시그니처 확정 → D near-term PoC 스코프 산정. (있으면 의외로 작음.)
   - (b) **전략 ADR 승격**: A·B·C·D·E를 ADR로 박아 다음 세션의 "3P가 핵심" 같은 잘못된 출발 차단.
3. **미해결**: E(포지셔닝 충돌) 해소 방향 확정, D 확장판의 LRU 파티션 캐시 현실성(PCIe 예산), 저선택성 임계값 추정.

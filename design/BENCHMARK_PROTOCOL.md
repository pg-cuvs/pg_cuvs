...
"GPU 빌드 → pgvector HNSW 서빙": 쿼리 축 동일 → 빌드 축만 비교. Ring A에 안 섞고 별도.
...
## 12. 재현성
- 고정·기록: pg_cuvs commit, 경쟁자 버전+커밋, PG·cuVS·CUDA·드라이버, 인스턴스·zone·단가, **hw_profile(probe된 link_bw/hbm_bw/ipc_rtt + env_tag)**.

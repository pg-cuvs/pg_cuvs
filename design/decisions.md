...
**상태**: 대체됨. CAGRA build/search/persistence PoC가 Phase 1 중 구현되었고, 최신 단계 관리는 ADR-015/016과 `design/specs/phase-record.md`를 따른다.
...
**문제**: `cuvs_ipc.c`에 IPC 프로토콜이 결정되지 않은 채 스캐폴드만 존재한다. phase-record.md는 "shm_open으로 핸들만 주고받는 방식"을 언급했지만 시그널링 메커니즘이 미결이었다.
...
**문제**: 초기 phase-record.md는 "CAGRA로 GPU 빌드 후 HNSW 포맷으로 변환"을 DiskANN 방식으로 언급했다. 그러나 cuVS에 더 직접적인 경로가 있는지 확인이 필요했다. Product Phase 2가 single-node CAGRA core로 고정되면서 DiskANN/Vamana는 Phase 3B로 이관됐다.
...
**결과**: phase-record.md의 CAGRA→HNSW 변환 방식은 cuVS `from_cagra()` API로 대체. 단, CAGRA→HNSW 경로도 CPU fallback용으로 병행 지원 가능 (`cuvs.export_hnsw = on` GUC).
...
- 인덱스 파일은 `pg_basebackup` physical base backup payload + WAL에서 제외 (`requirements.md OBJSTORE-03` 참조) **(2026-06-05 정정/주의: 기본 경로에서 미보장 — 단, 지역성과 직교)**:
  ...
  - **비용(실측 아님, 분석)**: 데이터 오염은 아니다(relfilenode fail-closed). 진짜 비용은 백업 비대 + **`pg_basebackup`으로 standby 신규 프로비저닝 시 쓸모없는 GB 전송 → RTO/네트워크 악화**. → P2 운영 하드닝(코드: index_dir이 DataDir 하위면 WARNING; 운영: 형제 디렉터리 프로비저닝 + 복원 시 경로 부재→rebuild). 기본값 변경은 권한 footgun(`postgres`가 PGDATA 밖 쓰기 불가) + breaking이라 보류, 기본은 유지하되 WARNING/playbook으로 유도. ROADMAP "운영 하드닝" wave + ops-gpu-playbook §6 참조.
...
**날짜**: 2026-05-23
**상태**: 보완됨. Phase 2/3 분리는 `design/specs/phase-record.md` 최신본을 따른다.
...
- ~~GPU kernel time, IPC time, CPU recheck time.~~ **(2026-06-05 정정: 미구현)** — 데몬은 단일 wall-clock `total_latency_us`만 기록하고(`src/pg_cuvs_server.c:135`), SRF에 `gpu_kernel_us`/`ipc_us`/`cpu_recheck_us` 컬럼이 없다. latency split은 phase-record.md(Phase 2 deferral)에서 보류로 명시했고, 외부 측정은 ADR-044(nsys/perf)로 수행했다.
...
3. 이 설정은 `make gpu-postinstall`(= `infra/scripts/setup/postinstall.sh`)이 idempotent하게 적용한다.
...
**결과 요약** (bench/results/competitive.csv, design/benchmarks/crossover-methodology.md §12):
...
**참조**: `crossover-methodology.md` §11(1M pilot), §12(50M competitive).
...
**배경**: Phase 3B spike 결과 (design/spikes/3b-diskann-spike.md):
...
- **문서화로 마무리**: spikes/3b-diskann-decision.md에 go/no-go 결론 명시
...
```
1. Competitive baseline 마무리 (crossover-methodology.md Cell B 완료, Cell C 선택적)
...
- 재시작 후 정상 동작 검증 (bench/legacy/test_3i_restart.sh)
...
**검증 결과 (bench/legacy/test_mig.sh, 2026-06-01)**:
...
PLAIN storage는 사용자가 선택적으로 적용할 수 있으므로, `docs/playbooks/` 또는 ops-gpu-playbook에 "빌드 성능 최적화 팁"으로 안내한다. 내용: "dim >= 512이고 빌드 빈도가 높은 경우, 벡터 컬럼에 `SET STORAGE PLAIN`을 적용하면 TOAST decompression 비용을 제거해 빌드 시간을 ~25-35% 줄일 수 있다. 단, 벡터를 참조하지 않는 일반 쿼리 성능이 저하될 수 있으므로 빌드 전용 테이블 또는 빌드 빈도가 높은 환경에서만 권장한다."
...
UNLOGGED + REINDEX 패턴을 ops-gpu-playbook에 추가하여 운영 패턴으로 권장.
...
- UNLOGGED + REINDEX 패턴이 현재 가장 빠른 실용 경로(~28s)이며 ops-gpu-playbook에 문서화됐다.
...
A100 HBM2e 대역폭(2TB/s) 기준 이론 하한값이다. Q=1 latency는 bandwidth-bound이므로 배칭 없이는 dim에 선형 비례한다. 참고: `docs/experiments/bruteforce-acceleration-lessons.md`.
...
- installcheck 10/10 PASS(`brute_force` 신규 테스트: L2/cosine recall@10=1.0, float16, sharded, bf_batch_wait_us 케이스(recall 안전망 + bf_batch_count 증가)). grouping 코어 유닛 테스트(`make test-unit`). 32-concurrent 무데드락 bench(`bench/legacy/bf_microbatch_concurrency.sh`).
...
**상태**: 측정 완료 (`docs/experiments/profiling-results.md`)

### 배경

pg_cuvs의 데이터 경로(빌드/검색/export)에서 PG backend - daemon - GPU 사이의 데이터 이동 비용이 정량적으로 측정되지 않은 상태다. Phase 2에서 `ipc_us`/`gpu_kernel_us`/`cpu_recheck_us` latency split을 의도적으로 deferred했고(phase-record.md Phase 2 의도적 deferral), 현재 daemon wall-clock latency 하나만 기록하고 있다.
...
- `docs/experiments/profiling-results.md`에 측정 환경/방법/결과를 아카이브한다.

### 측정 결과 (2026-06-05, A100-40GB / PG16 / N=1M dim=1024)

전체 수치·방법·제약은 `docs/experiments/profiling-results.md`. 요약:
...
측정 완료 (2026-06-05, A100-40GB / PG16 / N=1M dim=1024). 상세: `docs/experiments/profiling-results.md` §4.
...
- ops-gpu-playbook에 수동 정리 절차 추가: 데몬 정지 → 카탈로그에 없는 `<db>_<idx>.*`를 `sudo find ... -delete`(0700 디렉터리라 `rm glob` 무효) → 데몬 시작.
...
- **WARNING HINT 정정**(`src/pg_cuvs.c`): "until the daemon restarts" → "재시작이 정리하지 않고 좀비로 재로드; `pg_cuvs_gc_orphans(true)` 실행 또는 ops-gpu-playbook 수동 절차".
- **ops-gpu-playbook §6**: 자동(`pg_cuvs_gc_orphans`) + 수동(`sudo find ... -delete`) 정리 절차 추가.
...
**날짜**: 2026-06-06
**상태**: 구현·검증 완료 (설계 2026-06-06 → 구현 출고; installcheck 21/21 + isolation 2/2 GREEN. ROADMAP 완료표가 SSOT. 헤더 status 사후 정정 2026-07-13 — 하위 ADR-074/077이 'cagra 3Q extend 경로'를 출고 코드로 참조, cf. ADR-078)
...
**문제**: CAGRA 빌드 파라미터(`graph_degree`, `intermediate_graph_degree`, `build_algo`)가 cuVS 기본값에 고정되어 있다. 사용자가 recall↔build-time 트레이드오프를 제어할 수 없다. 4A preflight 측정(`docs/experiments/profiling-results.md`) 기준 GPU build ~68s가 천장인 상황에서 `graph_degree` 축소(32→16)로 build time과 VRAM을 모두 절감할 수 있으나 현재 수단이 없다.
...
> **검증 하네스**: `infra/scripts/tests/leak-verify.sh`(death-mode 매트릭스 + soak + 음성대조). 단위:
...
**날짜**: 2026-06-07
**상태**: 전략 방향 채택 (구현 미착수). 상세 분석·쟁점은 [design/strategy/workload-notes.md](strategy/workload-notes.md).
...
- **온라인 대용량 = 파티션 pruning + 기존 LRU** (정련, 스파이크 검증 2026-06-07): tenant LIST/RANGE 파티션 →
  ...
  **`MAX_INDEXES=64`가 하드월**(130 파티션 → 64만 resident=슬롯캡, VRAM 아님; 축출 파티션 쿼리는 ERROR+REINDEX 필요,
  ...
  → 수백+ 테넌트 온라인은 **선결: MAX_INDEXES 상향/동적화 + 런타임-축출 auto-reload 배선**(ROADMAP 백로그). 상세 workload-notes §G.
...
**미해결 쟁점**:
- ~~**포지셔닝 충돌(E)**~~ **해소됨(2026-06-07)**: `positioning.md`의 "exact GPU vector search" Avoid 문구는
...
- H streaming/out-of-core BF(미착수, 분석은 workload-notes §H): exact throughput 플레이, 온라인 단일-쿼리엔 부적합.
...
상세 계획: [docs/strategy/ecosystem-strategy.md](../docs/strategy/ecosystem-strategy.md)
...
**상태**: 1차 구현 완료 — 3O 역방향 맵 재활용, pread 기반 out-of-core gather + 청크 GPU BF + running top-k 머지. 분석 출처: workload-notes §H.
...
**날짜**: 2026-06-10
**상태**: ACCEPTED / 구현·검증 완료 (2026-06-11). Tier 1 `ci.yml`(CPU shim, 매 PR 자동·무료) + Tier 2 `gpu.yml`(UI 버튼 `workflow_dispatch` + WIF 키리스 GCP 인증, self-hosted A100, 실 installcheck 26/26) — PR #46–48, #50. 스펙: [design/ci-strategy.md](ci-strategy.md). 잔여: emulator 기반 GCS CI 회귀(트리거).
...
65번째 테넌트/파티션에서 막히면 첫 외부 사용자 이탈 사유(ADR-061 / workload-notes §G).
...
isolation 3/3 GREEN. 관련: [[ADR-061]](전략), workload-notes §G.

## ADR-069 — 벤치마크 실험 프로토콜 v2: 물리/판단 분리 + 코스트모델 보정 루프 선행

**날짜**: 2026-06-11
**상태**: ACCEPTED / 프로토콜 확정, 실행 미착수 (스펙: [design/benchmarks/protocol.md](benchmarks/protocol.md))
...
**날짜**: 2026-06-12
**상태**: ACCEPTED / 방향 기록 (실행은 ADR-026 트리거 충족 시). 근거: 3b-diskann-spike.md, ADR-025/026, ADR-061. **2026-07-24 개정 — 방향 3 철회, 신규 방향 4(포팅) 채택. 아래 "개정" 절 참조.**

**문제**: 3B(larger-than-RAM disk DiskANN cold tier)는 ADR-026에서 보류됐으나, 향후 트리거(1B+ 수요 / cuVS PQFlash stable / 128GB+ VRAM) 충족 시 "어떻게 구현하나"가 미기록 상태였다. 스파이크(3b-diskann-spike.md) 결과 cuVS 26.04는 Vamana **그래프 빌드는 정상**(in-memory recall 0.999)이나 **disk/PQFlash 직렬화가 PQFlash 비호환**(`sector_aligned` 출력이 1001 섹터 vs DiskANN 417 섹터 → PQFlash가 rerank 벡터를 잘못된 오프셋에서 읽어 recall 0.405로 붕괴). 또한 `cuvsVamanaSearch` 부재(Vamana는 build+serialize 전용, 검색은 MS DiskANN CPU에 의존). 이 갭을 닫는 길은 세 갈래이고, 그 선택과 기각을 고정한다.
...
**순위** (2026-06-12 원안): 방향 3(업스트림 기여) > 방향 1(advocate) > 방향 2(회피).

---

### 개정 (2026-07-24) — 방향 3 철회, 방향 4(AiSAQ 레이아웃 포팅) 채택

**본 ADR의 핵심 원칙은 유지된다** — "cold tier는 우리가 소유한다", "포크는 최후 수단",
"두 moving target을 만들지 않는다". 바뀐 것은 그 원칙을 만족하는 **경로**다. 상류 상황이
2026-06-12 이후 달라졌기 때문이다.

**개정 근거 — 1차 자료 확인 (2026-07-23, `pg_cuvs_docs/docs/ANN-SYSTEMS.md` 6.2·7.6.1)**

1. **방향 3의 대상이 동결됐다.** `microsoft/DiskANN` main은 **DiskANN3**(Rust,
   `DataProvider` trait 기반)로 재작성됐고, `PQFlashIndex`를 포함한 C++는 `cpp_main`
...
3. **AiSAQ는 우리가 산출물을 밀어 넣는 대상이 아니다.** cuVS integrations 페이지가
   ...
   워크플로 문서는 "A tool to convert from DiskANN index to AiSAQ one is not delivered"를
   명시하고, `build_disk_index`에 사전 계산 산출물 주입 플래그(codebook·그래프·단계
   건너뛰기)가 **없다**. 따라서 #1501 serializer를 "AiSAQ를 위해" 존치할 근거도 없다.
4. **그 cuVS 가속 AiSAQ 빌드는 공개 산출물에 없다.** Kioxia 정본(`aisaq_release`: CUDA
   흔적 0, `pushed_at` 2025-06-30), Knowhere `src/index/diskann/`(cuvs/cuda/gpu 참조 0,
   I/O는 `aio`/`uring`), Milvus 2.6.4+ 문서(GPU 언급 전무) — 셋 다 음성. Kioxia+NVIDIA의
   4.8B·7.8x 실측(2026-03-16)은 존재하나 코드 소재는 확인되지 않는다.

**방향 4 (신규, 채택) — AiSAQ 인라인 PQ 레이아웃을 자체 직렬화 계층으로 포팅**

AiSAQ를 조각내면 우리가 이미 가진 것과 없는 것이 갈린다.

| 조각 | 우리 상태 |
|---|---|
| Vamana 그래프 | cuVS 제공. in-memory 라운드트립 recall 0.999 (3b-diskann-spike.md) |
| PQ 코드북(k-means) | cuVS 제공 (`cuvs::cluster::kmeans`) |
| **인라인 PQ 디스크 레이아웃** | AiSAQ의 실제 기여. **직렬화 포맷 문제** |

AiSAQ 논문 자신이 "From Vamana graph and PQ vectors generated by DiskANN, a single AiSAQ
index file is generated"이라고 적듯 **새 그래프 알고리즘이 아니라 데이터 배치 기법**이다.
메커니즘은 각 노드 레코드에 이웃들의 PQ 벡터를 인라인해 "한 번 읽으면 다음 후보 평가에
필요한 것도 딸려 오게" 하는 것이며, 논문은 이를 DiskANN 블록의 **이미 비어 있던 영역을
채우는 것**("no additional I/O requests")으로 설명한다.

따라서 가져올 것은 **포맷 하나**이고, 이 프로젝트는 이미 직렬화 계층을 소유하고 있다
(`src/cuvs_util.c` 아티팩트 헤더·CRC, `src/cuvs_objstore.c` 스냅샷/복원). **남의 구현을
링크하는 것이 아니라 배치 기법을 우리 계층으로 옮긴다** — 임포트가 아니라 포팅이며,
이것이 본 ADR의 "우리가 소유한다" 원칙과 정합한다.

NVIDIA 권고와 충돌하지 않는다. "Vamana 직접 구현 말고"는 **그래프 알고리즘을 직접 만들지
말라**는 뜻이고, cuVS Vamana를 쓰는 이상 지켜진다. AiSAQ에서 가져오는 것은 알고리즘이
아니라 레이아웃이다.

**대안 기각 (개정 시점)**

- **Kioxia AiSAQ 링크 + 자체 패치**: 13개월 정지한 프로젝트를 포크하는 것. 리베이스
  부담은 없으나 버그·보안 수정을 전부 떠안는다 — 본 ADR이 방향 2를 기각한 사유보다 나쁘다.
- **Knowhere 링크 + 업스트림 기여**: 기여 자체는 포크가 아니고, Knowhere는 활발하며 같은
  저장소에 cuVS 통합(`src/common/cuvs/`, `src/index/gpu_cuvs/`)과 AiSAQ를 **이미 둘 다
  갖고 있다**(아직 연결되지 않음). 기각 사유는 기여가 아니라 **그 결과물을 쓰려면 Milvus의
  인덱스 라이브러리를 PG 확장에 링크해야 한다**는 점 — 의존성 추가가 아니라 아키텍처
  결정이다.
- **공개 대기**: 트리거가 아니라 외부 의존이다. 아래 트리거 개정 참조.

**트리거 개정** — 3B 재개 트리거에서 **"cuVS PQFlash stable"을 제거**한다. 두 번 틀렸다:
대상이 동결된 것이 첫째이고, 더 근본적으로 **남의 릴리스 일정에 우리 재개 시점을 묶은
형태**라는 것이 둘째다. 대체 트리거는 **"AiSAQ 레이아웃 포팅 착수 판단"** — 우리가 결정할
수 있는 것이어야 한다는 원칙을 만족한다. 나머지 둘(1B+ 수요, 128GB+ VRAM)은 유지.

> **귀속 정정**: 이 트리거는 **ADR-026 본문에 없다.** ADR-026의 재검토 조건은 "VRAM 128GB+
> 환경에서 100M+ 스케일 요구가 생길 때"뿐이다. "cuVS stable" 조건은 본 ADR의 문제 서술(위)과
> `ROADMAP.md` 3B 행, `design/specs/phase-record.md` Phase 3B 항목에서 자라난 것이며, 그 세 곳 중
> **로드맵 성격인 뒤의 둘을 이번 개정에서 함께 정리**했다. 본 ADR 문제 서술의 원문은
> 2026-06 시점의 기록이므로 그대로 둔다.

**순위 (개정)**: 방향 4(포팅) > 방향 1(advocate #2197, 기대값 하향) > 방향 3(철회) ·
방향 2(회피).

**남는 위험 (미해소)**

- **동등성 미보장**: 자체 포맷의 recall·지연시간이 AiSAQ 논문 수치에 못 미칠 수 있고
  비교 기준선이 없다. 논문 자신도 일부 데이터셋(SIFT1M, KILT E5)에서 DiskANN 대비 I/O
  레이턴시 저하를 인정하되 "not critical"로만 서술하고 수치를 제시하지 않는다.
- **포맷 유지보수**: 자체 포맷은 영구 소유한다. 다만 이는 이미 하는 일의 확장이지 새
  종류의 부채가 아니다.

**cuVS 이슈 처리**: #1501(PQFlash 호환 serializer)을 **cold-tier 재개 전제에서 제외**한다.
다만 #1501·#2197·#1380·#905의 재현·특성화 자체는 상류 기여로서 유효하며 대외 서사에서
유지한다. 철회하는 것은 "그 수정을 우리 콜드 티어의 전제로 삼는 것"이다.

**신규 관찰 대상 — DiskANN3 `DataProvider`** (보류): 상류가 trait 기반으로 전환하면서
파일 포맷을 바이트 단위로 맞추는 대신 인터페이스를 구현하는 경로가 생겼다. Bf-tree
provider가 "how to connect to a B-tree in your database" 예시로 제공되고, IP-DiskANN /
Fresh-DiskANN 기반 실시간 갱신과 속성 필터 훅을 표방한다 — 이 프로젝트의 미해결 축과
겹친다. **단 GPU와 cuVS가 어디에 들어가는지가 미해결이며, 채택 시 제품 정체성이 바뀔 수
있어 별도 전략 판단이 선행되어야 한다.** 현 시점 로드맵 항목 아님.
...
**복제쌍 인벤토리** (grep 앵커 `DUP-SIBLING(<key>)`; 각 key는 코드에서 정확히 2매치 = 원본 1 + 복제본 1. 완전성 대조: `grep -rhoE 'DUP-SIBLING\([a-z-]+\)' src/ | sort | uniq -c`):

| key          | 원본 (cagra)                 | 복제본 (flat)                | 갈리는 지점              |
|--------------|------------------------------|------------------------------|--------------------------|
| `build`      | `cuvs_ipc_build`             | `cuvs_ipc_build_flat`        | op code (`BUILD_FLAT`)   |
| `cost`       | `cuvsamcostestimate`         | `flat_amcostestimate`        | exact BF N-cost 강제     |
| `build-heap` | `cuvs_build_cagra_from_heap` | `cuvs_build_flat_from_heap`  | 그래프 빌드 없음         |
| `ambuild`    | `cuvs_ambuild`               | `flat_ambuild`               | vectors-only 저장        |
| `gettuple`   | `cuvs_gettuple`              | `flat_gettuple`              | `search_mode=1` 강제     |

이 표(what)는 위 원칙(why)의 부속 원장이다. 새 복제쌍 추가/삭제 시: 양쪽 함수에 `DUP-SIBLING(<newkey>)` 태그를 달고 이 표를 갱신한다. **한계**: grep 앵커는 쌍의 *등록 완전성*(짝 누락·표↔코드 목록 불일치)만 감사한다 — 두 복제본의 *내용 동기화*는 회귀 펜스(`brute_force`, `flat_smoke` 등)가 지킨다.
...
## ADR-078 — Corey cuVS+Lucene 강연 외부 검증 및 도출 액션
...
2. **병합(merge)이 최대 미결**: trivial(원바이트 통째 보관, 쿼리 저하) vs non-trivial(원벡터 재인덱싱, 비용↑). 발표자 표현 그대로 "workaround" — 근본 알고리즘/구현 미보유.
3. **I/O 병목 이동**: 인덱싱을 GPU로 옮기니 병목이 disk/network ingest로 이동. 대응책으로 RAPIDS Quick I/O(cuFile/GPUDirect Storage)·PCIe CPU-bypass 검토 중.
4. **배치/동적 쿼리**: GPU 검색 잠재력을 풀려면 dynamic batching 필수인데, Lucene API 구조가 이를 막아 커뮤니티 논의 필요라고 공개 토로.
5. **생태계**: Q&A 통합 대상 나열(Milvus/FAISS/OpenSearch/Qdrant/Oracle)에 **Postgres 없음**. 팀은 마케팅보다 재현가능 벤치마크·작동 코드에 반응.

**ADR 매핑 판정 (2026-07-13)**:

| 강연 사실 | 우리 상태 | 판정 |
|---|---|---|
| CAGRA→HNSW | ADR-029/036/037/038/041 실측(import_hnsw 100K **15.8x**/1M **13.9x**)+DDL 제품화, ADR-032 사이드카 조건부화 | **지지+선점**(그들 정성/우리 실측) |
| merge 미결 | **3Q 완료**(ADR-051 `cuvsCagraExtend`/`cuvsCagraMerge`, installcheck 21/21 + iso 2/2) + 4C(ADR-050) | **이미 해결**(그들은 워크어라운드) |
| I/O 병목 | **4A 완료**(ADR-057/058/059 memfd 무복사, copy≈0) | 동일 병목 대응. GDS/cuFile은 미기재 |
| 배치 쿼리 | **3M 완료**(ADR-040 `pg_cuvs_batch_search`)+ADR-039 마이크로배칭 | **이미 해결**(Lucene API 제약과 대비되는 우위) |
| 생태계 Postgres 부재 | ADR-062 | 전제 직접 확증. FFI(JNI→Panama)는 우리 무관(PG C 네이티브) |

**정정**: 초안 분석은 ADR-051(3Q)을 "설계 단계"로 판단했으나(본 decisions.md의 ADR-051 헤더 status가 구현 후 미갱신 stale), ROADMAP 완료표·에코시스템표·하위 ADR 참조(ADR-074/077의 "cagra 3Q extend 경로 상속") 기준 **3Q는 구현 완료**(installcheck 21/21 + iso 2/2 GREEN). 따라서 "3Q 우선순위 상향"은 무효이며, 오히려 pg_cuvs가 Lucene 미결(merge)을 **이미 닫았음**이 확인됨. (ADR-051 헤더 status 드리프트는 2026-07-13 '구현·검증 완료'로 별도 정정함.)

**도출 액션**:
1. **에코시스템 Stage 2 보강 (1순위, 이미 착수 가능)**: 강연이 merge(3Q로 해결)·batch(3M로 해결)를 Lucene 커넥터의 **공개 미결**로 1차 사료 확증. → cuvs-bench backend PR 피치에서 "pg_cuvs = cuVS streaming-update API(`cuvsCagraExtend`/`cuvsCagraMerge`)를 DB 레벨에서 실사용하는 유일 사례"를 핵심 근거로 활용. ROADMAP "cuVS 에코시스템 진입 계획"에 반영.
...
**raw 타이밍 정책 조건부 대체 (2026-08-05, #98)**: 위 **결정 2의 "raw 라이브러리 타이밍은 사용하지 않는다"** 는 [ADR-084](#adr-084--raw-cuvs-타이밍의-조건부-보고-adr-080-결정-2를-금지에서-같은-run-앵커로만-허용으로)가 조건부로 대체한다 — 같은 프로세스·같은 인덱스 파라미터·같은 CSV에서 나오고, `axis` 라벨을 달고, 지연 축에서는 통합 비용 앵커로만 인용될 때에 한해 보고한다(축 교차 비율은 여전히 금지). 결정 2의 나머지(측정 경계 = end-to-end psql 왕복, 헤드라인은 end-to-end만)와 결정 1·3·4는 변경 없이 유효하다.
...
**배경**: ADR-079는 VecFlow(arXiv:2506.00812, cuVS 엔지니어 저술)를 근거로 **3O(글로벌 CAGRA 그래프 + GPU BITSET prefilter)가 recall ~80%에서 정체하고 선택적 필터에서 0%로 붕괴**할 리스크를 제기했다. pg_cuvs는 3O를 출하했으나 **자체 구현에서 이 실패 모드를 한 번도 재보지 않았다.** 이슈 #76은 이를 "최우선·저비용"으로 표시했다.

**측정 방법(정직성 장치 2개)**: (a) ground truth를 **필터 부분집합에 대한 exact top-k로 numpy에서 독립 계산** — pg_cuvs를 기준값으로 쓰지 않으므로 엔진의 계통 오차가 기준에 숨을 수 없다. (b) 데몬의 `search_mode`를 매 지점 읽어 기록 — 3O가 내부적으로 정확 BF prefilter(mode 3)나 D-wedge로 폴백한 실행을 3O 결과로 오인하지 않는다. 전 지점에서 `cagra_prefilter`(mode 4)가 확인됐다.

| selectivity | \|S\| | 3O recall@10 | 3O p50 | D-wedge recall@10 | D-wedge 반환행 | D-wedge p50 |
|---:|---:|---:|---:|---:|---:|---:|
| 0.5   | 500,000 | 0.986 | 38.8 ms | 1.000 | 10.00 | 13.8 ms |
| 0.1   | 100,000 | 0.996 | 8.7 ms  | 0.999 |  9.99 | 4.4 ms |
| 0.05  |  50,000 | 0.995 | 5.3 ms  | **0.782** | 7.82 | 3.5 ms |
| 0.01  |  10,000 | 0.994 | 2.3 ms  | **0.164** | 1.64 | 2.8 ms |
| 0.005 |   5,000 | 0.995 | 1.9 ms  | **0.079** | 0.79 | 2.6 ms |
| 0.001 |   1,000 | 0.983 | 2.5 ms  | **0.011** | 0.11 | 2.8 ms |

**결론**:
1. **3O에서 VecFlow형 붕괴는 재현된다 — 단 무릎이 0.001 아래에 있다.** 최초 스윕의 최저점이 하필 0.001이라 무릎 직전에서 멈췄고, 그래서 "재현되지 않았다"는 잘못된 1차 결론을 냈다. 격자를 아래로 연장한 결과:

| selectivity | 3O recall@10 | 3O 반환행 | 3O p50 |
|---:|---:|---:|---:|
| 0.002  | 0.994 | 10.00 | 2.35 ms |
| 0.001  | 0.986 |  9.98 | 2.42 ms |
| 0.0005 | **0.857** |  9.91 | 2.36 ms |
| 0.0002 | **0.475** |  6.54 | 2.31 ms |
| 0.0001 | **0.273** |  3.29 | 2.28 ms |
...
2. **D-wedge도 붕괴했고, 원인은 코드에 그대로 있었다 — #80은 무상관 fixture의 직접 원인을 제거했다.** 수정 전 선택도 0.05 이하에서 recall이 0.78 → 0.16 → 0.08 → 0.01로 무너지고 반환 행 수가 0.11개까지 떨어졌다. 원인은 데몬의 `bk_target = k*4` — **선택도와 무관한 고정 오버페치** 후 후처리 필터다. 거리와 무상관인 필터에서 j번째 필터 이웃의 기대 미필터 랭크는 ~j/sel이므로 깊이를 1/sel로 스케일하는 `bk_target = max(k*4, k/sel·2)`로 바꿨다. 같은 무상관 fixture를 재측정한 결과:

| selectivity | D-wedge recall (수정 전 → 후) | 반환행 | p50 (전 → 후) |
|---:|---:|---:|---:|
| 0.05   | 0.782 → **1.000** | 10.00 | 3.55 → 3.47 ms |
| 0.01   | 0.164 → **1.000** | 10.00 | 2.77 → 3.49 ms |
| 0.001  | 0.011 → **1.000** | 10.00 | 2.78 → 7.25 ms |
| 0.0001 | (미측정) → **1.000** | 10.00 | — → 35.16 ms |

   이 표는 **무상관 해시 fixture에서 recall 1.000을 관측했다는 결과**이지, 임의의 필터-거리 관계에 대한 exactness 증명이 아니다. D-wedge는 전체 거리 중 앞 `bk_target`개만 뽑은 뒤 whitelist를 적용하므로, 허용 행이 그 prefix 밖에 몰리는 반상관 입력에서는 비례 오버페치도 정답을 놓칠 수 있다.

3. **측정한 무상관 fixture에서는 두 경로의 거래 구조가 뒤바뀌었다.** #80 이후 이 fixture의 D-wedge는 전 구간 recall 1.000이고 3O는 어디서도 더 정확하지 않다. 3O가 더 빠른 구간은 (a) 차이가 5 ms 미만이거나(sel 0.001~0.01), (b) recall이 무너진 구간(sel < 0.001)뿐이다. 이 결론의 범위는 아래 상관 한계까지이며, 임의 입력에 대한 D-wedge exactness 주장은 철회한다.

| | 3O | D-wedge (#80 이후) |
|---|---|---|
| recall | sel<0.001에서 붕괴 (0.86→0.27) | 측정한 무상관 격자에서 1.000 |
| p50 | 평탄 ~2.3 ms | 1/sel에 비례 (3.5 → 35 ms) |

4. **고선택도에서는 3O가 오히려 느리다** (sel 0.5에서 40.5 ms vs 13.1 ms — 50만 TID의 BITSET 구성 비용). 따라서 라우팅 임계는 "recall을 지연과 바꾸는 지점"이 아니라, **정확성을 지연과 바꾸는 지점**이 된다.

**도출 액션**:
1. **완료(#80)**: D-wedge 오버페치를 선택도 비례로. 붕괴의 직접 원인이었다.
2. **`cuvs.filter_auto_threshold` 기본값 재검토(#81)** — 최초 권고("0.05 → 0.1 상향")는 #80 이전 데이터에 기반한 것이라 **철회한다.** #80 이후 데이터는 반대 방향을 가리킨다: 현재 기본값 0.05는 sel 0.001~0.05 구간을 3O로 보내는데, 그 구간에서 3O는 D-wedge보다 덜 정확하면서(0.99 vs 1.000) 빠르지도 않다. 기본값을 내리는(극단적으로 0 = 항상 D-wedge) 방향이 데이터가 지지하는 선택이나, 출하된 기능의 기본 동작을 바꾸는 결정이므로 별도 판단.
3. **GUC 설명문 정정** — "3O gives recall=1.00 at any selectivity"는 mode 3(BF prefilter)의 성질을 mode 4(CAGRA prefilter)에 잘못 붙인 것이다.
4. **극저선택도의 제3 경로 측정** — 최초 두 경로 스윕에는 stream BF가 없었지만 후속 three-path 실행으로 측정했다. `bench/results/adr079_3paths_verified.csv`에서 stream BF는 sel 0.001/0.0002/0.0001에 recall 1.000, p50 1.689/0.781/0.649 ms였다. 반대로 sel 0.05에서는 139.507 ms로 D-wedge 3.560 ms보다 크게 느렸다. `bench/results/adr079_3o_recall_crossover.csv`의 교차 구간은 다음과 같다.

| selectivity | D-wedge p50 | stream BF p50 | 더 빠른 exact 관측 경로 |
|---:|---:|---:|---|
| 0.008 | 3.631 ms | 12.902 ms | D-wedge |
| 0.005 | 3.853 ms | 6.453 ms | D-wedge |
| 0.003 | 4.270 ms | 3.131 ms | stream BF |
| 0.002 | 5.172 ms | 2.570 ms | stream BF |

   따라서 이 데이터셋의 관측 교차점은 0.003과 0.005 사이이며, 0.004 기본값은 그 사이를 택한 운영값이다. 이 수치는 단일 호스트·단일 데이터셋 결과이므로 보편 상수가 아니다.
...
correlation에 대한 서술을 두 번 고쳤고, 최종 형태는 다음과 같다. 축은 하나가 아니라 **부호가 있다.**

| 필터-거리 관계 | D-wedge(고정 오버페치) | 근거 |
|---|---|---|
| 양의 상관(쿼리 근처만 통과) | 쉬움 — 1% 선택도에서도 recall 1.00 | `docs/experiments/filter-threshold-experiment.md` |
| 무상관(본 측정의 해시) | 중간 — 1%에서 0.20 | 같은 문서 |
| **반상관(먼 행만 통과)** | **최악 — 프리픽스에 통과 행이 0개일 수 있음** | 2차 리뷰 지적, #86에서 재현 |

즉 초판의 "상관이 더 가혹하다"도, 2판의 "무상관이 최악이다"도 틀렸다. 무상관은 양의 상관보다 나쁘지만 최악은 아니며, **반상관이 최악**이다. 이 조건에서 고정·비례 어느 오버페치도 exact를 보장하지 못한다. #86의 후속 전수 재검색은 반상관 fixture에서 10/10을 회복했지만, 코퍼스 크기 출력 버퍼와 `k=N` GPU 작업을 요구하므로 bounded exact 구현으로 교체되기 전에는 일반 해법으로 간주하지 않는다.
...
## ADR-084 — raw cuVS 타이밍의 조건부 보고: ADR-080 결정 2를 "금지"에서 "같은-run 앵커로만 허용"으로

**날짜**: 2026-08-05
**상태**: 결정 (#98 PR-B 구현 완료; 측정 run은 PR-D2)

**supersedes**: ADR-080 결정 2 중 "Postgres를 제외한 raw 라이브러리 타이밍은 pg_cuvs 수치가 아니므로
사용하지 않는다" 부분을 **조건부로** 대체한다. 나머지(측정 경계 = end-to-end psql 왕복, 헤드라인은
end-to-end)는 **그대로 유효**하다. 현행 SSOT는 이 ADR + `bench/cuvs_bench_backend/README.md`.

**배경**: ADR-080이 raw 타이밍을 금지한 이유는 정확했다 — 당시 유통되던 "24× / 12 s"는 Postgres의
COPY·WAL·페이지 물성화를 제외한 다른 경계의 수치였고, end-to-end 수치와 **다른 run·다른 날·다른
호스트**에서 나와 뺄 수도 나눌 수도 없었다. 금지는 그 특정 실패(이어붙이기)를 막기 위한 조치였지,
...
1. **raw 타이밍은 다음 4조건을 모두 만족할 때에만 보고한다.**
   - (a) **같은 프로세스·같은 인덱스 파라미터·같은 CSV** — SQL 행과 같은 run에서 나온다.
     (`bench/cuvs_bench_backend/cuvs_engine.py`가 `PgEngine`과 동일 표면으로 같은 하네스 안에서 돈다.)
   - (b) **축 라벨 필수** — 모든 행에 `axis` 컬럼. raw 행은 `notes`에 `raw-arm=wall-clock`.
   - (c) **용도 한정** — 지연 축에서는 **통합 비용 앵커**로만, 처리량 축에서는 **라이브러리
     메커니즘**(`cuvs_batch`)으로만 인용한다.
   - (d) **축 교차 비율 금지** — 지연 축 raw ÷ 처리량 축 SQL 같은 비율은 어떤 형태로도 계산·발표하지
     않는다. 이 금지는 ADR-080의 취지를 그대로 승계한 것이다.

2. **헤드라인은 여전히 end-to-end다.** raw 수치는 헤드라인·재프레이밍·배율 승격의 재료가 아니다.
   "raw가 X µs이므로 pg_cuvs의 진짜 속도는 X다" 류의 문장은 이 ADR로도 여전히 금지된다.

3. **지연 축 `cuvs` 행 = python 디스패치 포함 host wall-clock이며 §1.1의 715 µs와는 다른 양(量)이다.**
   §1.1은 nsys 커널 시간 합이다. 따라서 이 행은 §1.1을 **대체하지 않고 병기**하며, §1.1의 커널 시간
   갱신은 별도로 nsys가 담당한다(#98 PR-E). 추가로 raw 행은 쿼리의 host→device 전송을 타이밍 창에서
   제외하므로(선례 `bench/legacy/anbench/run_cuvs.py:84-89`) **하한**이다 — 통합 비용을 과대평가하지
   않는 방향의 보수적 편향임을 명시한다.

4. **파라미터 정합이 인용의 전제다.** raw 암은 `pgcuvs_cagra`와 동일한 build grid
   (graph_degree {32,64,128}, intermediate 128 고정)와 동일한 검색 스윕(16/32/64/100/200/400)을 쓰고,
   itopk_size도 확장이 유도하는 규칙(`src/cuvs_wrapper.cu:1122-1124`: 32의 배수로 올림, 하한 64)을
   그대로 복제한다. **`build_algo`도 양 암에서 `ivf_pq`로 고정한다** — 확장의 reloption 기본값은
   `auto`(코퍼스 크기 휴리스틱으로 ivf_pq/nn_descent 중 택일, `docs/reference.md:262`)인데 raw 암은
   알고리즘을 명시하므로, `auto`를 두면 두 암이 서로 **다른 알고리즘으로 만든 그래프**를 비교하고
   있을 수 있다. 스윕 차원이 아니라 **제거되는 변인**이며, 재사용 게이트의 `expected_reloptions`도
   이 값을 요구한다(불일치 시 재빌드). 이 정합이 깨지면 두 행의 차는 통합 비용이 아니라 빌드/튜닝
   차가 되므로, 인용 자격도 함께 사라진다.

5. **raw 암은 두 번째 GPU 테넌트다.** 인덱스가 하네스 프로세스의 VRAM에 상주하므로 데몬과 공존한다.
   전 raw 행에 `nvidia-smi memory.used` 판독을 `notes`로 동반 기록해 그 사실이 CSV에서 보이게 한다.

**구현**: `bench/cuvs_bench_backend/cuvs_engine.py`(신규, cupy/cuvs lazy import → CPU에서 import 가능),
`backend.py`의 `_engine_for()` 디스패치(raw는 SQL 릴레이션 게이트 우회 — 대조할 `pg_class` 행 자체가
없다; 재사용 판정은 **프로세스 내 엔진 상태만**을 근거로 한다. 사이드카 레코드는 프로세스 재시작을
살아남지만 GPU 인덱스는 그렇지 않으므로, 레코드 단독 재사용은 사라진 인덱스에 낡은 build_time을
귀속시킨다), `sidecar.py`의 `BUILD_GRIDS["cuvs"]`, `pg_engine.DEFAULT_SWEEPS["cuvs"]`.
`cuvs_batch`(단일 디스패치 배치)는 `CuvsEngine.search_batch()`로 존재하나 오케스트레이터에 연결되지
않는다 — 처리량 축 배선은 #98 PR-C 소관.

**대안과 기각 사유**:

| 대안 | 기각 사유 |
|---|---|
| 금지 유지 (raw 미측정) | Gap A가 영구 미해결. 통합 비용을 말할 근거가 계속 없고, §1.1의 이어붙인 잔차가 그대로 남는다 |
| 별도 run·별도 CSV로 raw 측정 | 지금 문제의 원인이 정확히 그 형태다. 호스트·날짜 편차(§2.1b 3.5×)가 차이를 삼킨다 |
| raw를 처리량 축에만 허용 | 통합 비용은 지연 축 질문이다. 배치만으로는 "쿼리 1건의 SQL 비용"에 답할 수 없다 |
| §1.1을 raw wall-clock으로 갱신 | 다른 양이다. 커널 시간 자리에 wall-clock을 넣으면 §1.1이 조용히 오염된다 (PR-E nsys 소관) |

**측정 완료 (2026-08-05, #98 Stage 2 — append)**: 위 설계대로 1M 본 run이 완주했다
([`bench/results/pg_cuvsbench_98.csv`](../bench/results/pg_cuvsbench_98.csv),
`massedcompute_A100_sxm4_80G_DGX`, wiki_all_1M 1M×768). 88 지연 행 + 12 처리량 행, 전 행
`success=True`, 세그먼트 게이트 14/14 통과, conc 10행 fallback 델타 전부 0.

- **Gap A 해소치**: recall≥0.95에서 raw `cuvs` 708.3 QPS vs `pgcuvs_cagra` 580.5 QPS =
  **1.22×**, 즉 SQL 왕복 통합 비용 **0.31 ms/query**, raw 대비 처리량 82% 유지. 결정 3대로
  이 값은 지연 축 앵커로만 인용하며 §1.1의 715 µs 커널 합과 나누지 않는다(다른 量).
- **결정 5 이행 확인**: 전 raw 빌드에 `nvidia-smi` 판독 동반(델타 120–486 MB).
- **`cuvs_batch` 배선 완료**: PR-C가 처리량 축에 연결했고 이 run에서 70,990 QPS @ recall
  0.9892로 측정됐다. 처리량 축 행이므로 위 1.22×와 나누지 않는다.
- **부수 관측(이 ADR의 결정을 바꾸지 않음)**: 배치-단건 recall 발산이 1M에서 same-graph
  0.00225–0.00250으로 100k(0.00065–0.0010) 대비 확대됐고, raw `cuvs_batch`도 같은 방향
  (0.9892)이라 소재는 cuVS 커널 분기 쪽이다. 보정 게이트는 관찰-기록으로 강등됐고,
  제품 관찰은 **[#144](https://github.com/pg-cuvs/pg_cuvs/issues/144)** 로 분리 등록됐다.
  raw 암이 같은 방향을 보인다는 사실은 이 ADR의 결정 3(raw = 통합 비용 앵커)을 오히려
  뒷받침한다 — 발산이 SQL 경로 고유가 아니므로 1.22× 앵커의 해석은 영향받지 않는다.

**남는 위험**: (a) 독자가 raw 행을 "pg_cuvs의 속도"로 오독 — 축 라벨·notes·리포트 각주 3중으로 방어
하지만 인용은 통제 밖이다. (b) wall-clock 하한 편향의 크기 미정량 — H2D 전송을 포함한 변형 측정은
이번 범위 밖. (c) VRAM 이중 테넌트가 데몬 성능에 주는 영향은 미분리(로그로 관측만).

**관련**: ADR-080(대체 대상 + 나머지 유효), ADR-069(벤치 방법론), ADR-062(에코시스템 Stage 2),
#98(측정 설계 — 두 축, 축내 비교만).

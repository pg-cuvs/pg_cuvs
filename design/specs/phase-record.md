# pg_cuvs Phase 구현 기록 (as-built, 동결)
...
> 새 계획은 phase-record.md가 아니라 해당 트랙 문서로 간다(벤치마크=[design/benchmarks/protocol.md](../benchmarks/protocol.md), 순차/트리거=[ROADMAP.md](../../ROADMAP.md)).
...
- `cuvs.k`, opclass/opfamily metric identity, and k-dominant cost model (legacy fallback; superseded by the ADR-075 physical cost model when the hardware profile is probed — see "코스트 모델 v2" below).
...
**2026-05-30~31 Competitive Benchmark 결과 (crossover-methodology.md §11~12)**:
...
- **Phase 3B go/no-go**: NO-GO. cuVS PQFlash 경로 미완성, DiskANN 50M에서 실용 불가.
  pg_cuvs = GPU VRAM hot tier 포지셔닝 고정. 재검토 조건: 1B+ 수요 확인,
  **AiSAQ 레이아웃 포팅 착수 판단**, 또는 128GB+ VRAM.
  (design/spikes/3b-diskann-decision.md, ADR-026 참조)
  - **2026-07-24 개정**: 재검토 조건에서 **"cuVS stable" 제거**. MS DiskANN C++가
    `cpp_main`으로 동결되어 대상이 유지보수되지 않고, 더 근본적으로 남의 릴리스 일정에
    재개 시점을 묶는 형태였다. 재개 시 구현 방향은 ADR-072 개정 절 참조 —
    남의 구현을 링크하지 않고 AiSAQ 인라인 PQ 레이아웃을 자체 직렬화 계층으로 포팅한다.
...
이 절의 Phase 2는 제품 로드맵 단계다. 목표는 단일 PostgreSQL instance + GPU sidecar 조합을 "데모"가 아니라 운영 가능한 단일 노드 엔진으로 만드는 것이었다. 2026-05-26 기준 **single-node core 완료**(stale 자동 CPU fallback은 Phase 2.1에서 마저 배선). 완료 판정과 증거는 `docs/history/phase2-exit-criteria.md`, feature별 테스트 매핑은 `docs/history/phase2-test-matrix.md`를 따른다.
...
- **Step 3 — Planner/cost model**: per-tuple 중심 cost를 k-dominant model로 교체하고, plan-shape regression으로 GPU path 선택과 `enable_cuvs=off` 회피를 잠갔다. JIT 설정은 변경하지 않았다. (이 k-dominant model은 ADR-075 물리 코스트 모델의 레거시 폴백으로 남았다 — 아래 "코스트 모델 v2" 참조.)
...
- **Step 7 — Test matrix**: unit/regress/integration/e2e coverage를 `docs/history/phase2-test-matrix.md`에 정리했다. Integration scenarios 8-12가 observability, metric/k, staleness, build memory cap, cache eviction/reload를 검증한다.
- **Step 8 — Exit criteria audit**: `docs/history/phase2-exit-criteria.md`의 10개 기준 전부 `MET`(stale fallback 기준 #5는 Phase 2.1 hotfix 후 MET 복구).
...
- `docs/history/phase2-exit-criteria.md`의 criterion #5를 `MET`으로 되돌릴 수 있다.
...
- PLAIN storage(`ALTER TABLE ... SET STORAGE PLAIN`)는 사용자 선택이며, 빌드 성능 개선이 가능하나 일반 쿼리 성능 저하 트레이드오프가 있다. ops-gpu-playbook에 "빌드 성능 최적화 팁"으로 안내한다. *(이 단락의 "~45s PG 오버헤드"·"~25-35% 개선"은 ADR-044 프로파일링 이전 추정 — 실측은 backend ~15.5s, PLAIN 빌드 절감 ~8%. `docs/best-practices.md` §1 참조.)*
...
| 목표 | 수치 | 검증 방법 |
|------|------|-----------|
| 4A 완료 (ADR-057/058/059) | backend 오버헤드 ~6.3s(단일) → ~3.7s(병렬). 원안 wall-clock 목표(4A-1 ≤50s / 4A-2 ≤35s)는 ADR-044 실측(GPU floor ~33–68s 지배)으로 도달 불가가 확인돼 폐기 — north-star는 backend 오버헤드 제거율 | `docs/experiments/profiling-results.md` §7/8/9 + self-NN 단일==병렬 5/5 + installcheck 15/15 + iso 2/2 |
...
**완료** — installcheck 20/20 + isolation 2/2 PASS (ADR-049)
...
## 코스트 모델 v2 — 하드웨어 앵커 물리 비용 (ADR-075)

레거시 코스트는 행 수 휴리스틱 상수(`CUVS_STARTUP_COST=1000`/`CUVS_FLAT_STARTUP_COST=50`, k-dominant)였고 특정 장비(A100/PCIe)에 굳어 있었다. ADR-074 측정이 보여준 실제 동인은 데이터 이동(detoast·H2D·HBM 대역폭·IPC)이다. v2는 cagra/flat 비용을 **배포 장비에서 측정한 물리 계수**로 매긴다.

### Phase 1 — 하드웨어 probe + CRC 사이드카 + read-only view (구현 완료)
- 데몬이 부팅 시 `link_bw`/`hbm_bw`/`gpu_bf_tput`/`ipc_rtt`/`cpu_dist_tput`/`gpu_cagra_lat_us`를 실측 → `CuvsHwProfile` v2(136→152B, magic+version+CRC, 정체성 태그) 원자적 기록(`<index_dir>/cuvs_hw_profile`). 버전 조건부 read(v1 사이드카는 신규 필드 DEFAULT). 플래너는 plan-time에 싸게 read(CUDA/IPC 금지). `pg_cuvs_hw_profile()` SRF로 노출. 라우팅 무변경.

### Phase 2 — 물리 비용 공식 (구현 완료)
- **앵커**: `κ = cpu_operator_cost · cpu_dist_tput / dim` (GPU µs를 코어 seqscan의 PG cost-unit으로 환산; 코어가 거리 연산을 dim-blind flat per-call로 매기므로 κ ∝ 1/dim이 고차원에서 GPU를 더 일찍 이기게 함).
- **cagra**(graph): `startup = κ·(ipc_rtt + gpu_cagra_lat_us)`, `total = startup·(1 + cuvs_k/100)` (N-독립). **flat**: `startup = κ·ipc_rtt`, `total += κ·(N·dim·4 / hbm_bw)` (HBM 스트림). brute_force search_mode 분기는 레거시 유지.
- **무회귀 게이트**: `cuvs.enable_phys_cost`(기본 on) ∧ 프로파일 로드 성공 ∧ `(probe_status & 필요비트)==필요비트`일 때만 물리; 아니면 레거시 상수 **바이트 동일**(Tier-1 shim은 GPU 계수 미프로브 → 필요 GPU 비트 미충족 / 미프로브 / GUC off). dim은 plan-time syscache typmod(`table_open` 없음). 방어: κ NaN/Inf/≤0 거부, hbm_bw 타당범위 클램프 + isfinite 가드, probe try/catch(데몬 abort 방지).

### Phase 3 — transient B 물리화 + `auto` (보류 — 장비 블록)
- PCIe에선 transient B 잉여(ADR-074). `auto` 물리 공식 배선 + `link_bw` 밴드 게이트 + `bf_tput` 검색-전용 정밀화는 통합메모리(GH200/MI300A) 확보 시. ROADMAP 트리거 백로그.

### 완료 기준 / 검증 증거 (Phase 1+2, A100 VM)
- probe_status=0x3f, 사이드카 152B. `pg_cuvs_hw_profile()` source=measured/matches=true.
- 판별 플립(`routing_golden_measured`): dim8 N=10000 → 레거시 seqscan·물리 cagra(배포별 교차점 교정), N=2000 anti-flip(둘 다 seqscan).
- 레거시 복귀 3경로(GUC off / 사이드카 부재 / Tier-1) 바이트 동일. installcheck 35/35 + isolation 6/6 GREEN.

스펙: ADR-075(설계 + Phase 2 구현절). 테스트: `test/sql/hw_profile.sql`, `routing_golden.sql`(Tier-portable), `routing_golden_measured.sql`(Tier-2).
...
목표: cuVS 공식 문서/README에 pg_cuvs 등재 + cuvs-bench backend 코드 기여. (ADR-062)
...
| 재현 가능한 벤치마크 | `BENCHMARK.md` 공개 (build 13×, latency 11× 등 핵심 수치) |
| 외부 설치 가이드 | README 정비 |
| 기본 CI | GitHub Actions 최소 구성 |

### 단계별 계획

**1단계 — repo 공개 + 벤치마크 공개** (즉시 가능)
- repo public 전환, `BENCHMARK.md` 공개, README 정비.

**2단계 — cuvs-bench backend PR** (3Q 완료 후)
- cuvs-bench pluggable backend API로 `pg_cuvs` backend 구현 → PR.
- 3Q 완료 시 `cuvsCagraExtend`/`cuvsCagraMerge`를 PG에서 활용하는 유일한 사례.
- Elasticsearch backend PR이 선례.

**3단계 — cuVS 문서/README 링크 요청** (2단계 merge 후)
- GitHub Discussion 또는 PR로 Milvus/Kinetica와 동일 수준 링크 요청.

**4단계 — NVIDIA 채널 노출** (3단계 후)
- RAPIDS 뉴스레터, NVIDIA 개발자 블로그, PostgreSQL 컨퍼런스 연계.

### 완료 기준

- cuVS 공식 문서 또는 README에 pg_cuvs 링크 등재 확인.
- cuvs-bench에 pg_cuvs backend merge 확인.

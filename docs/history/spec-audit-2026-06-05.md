...
ADR-009 사건(설계 문서가 "완료"로 적었으나 미구현 → orphan 누수의 근본 원인)을 계기로, `design/decisions.md`의 각 ADR "결과/결정/구현현황"과 `design/specs/phase-record.md`의 "완료 기준"이 주장하는 동작이 실제 소스에 있는지 grep/read로 교차검증했다. 핵심 의심 휴리스틱: (a) standalone sidecar 데몬(PG 연결 없음, ADR-002)이 할 수 없는 동작 주장, (b) positive 테스트만 있고 negative/cleanup/failure-path 테스트가 없는 기능.
...
|---|---|---|---|---|---|---|
| 1 | `pg_stat_gpu_search`가 GPU kernel / IPC / CPU recheck time을 노출 | ADR-017 (초기 지표) | SRF는 `avg/p50/p95/p99` wall-clock만(`sql/pg_cuvs--0.1.0.sql:144-`), 데몬은 단일 `total_latency_us`(`src/pg_cuvs_server.c:135`). phase-record.md도 split "아직 없다" | OVERSTATED | med | ADR-017 정정 (deferred 명시, ADR-044 외부측정 cross-ref) |
| 2 | `pg_stat_gpu_search`가 fallback count/reason 노출 | ADR-017; phase-record.md:177 | 데몬 struct에 fallback/success 카운터 없음(`src/pg_cuvs_server.c:133-134`), `grep fallback_count`=0. fallback은 백엔드 plan-time 결정(`src/pg_cuvs.c:932`)이라 데몬 미도달 | NOT-IMPLEMENTED | med | ADR-017 정정 (구조적 미관측 명시) |
...
| 6 | OBJSTORE-03: 인덱스 파일은 `pg_basebackup`에서 제외 | ADR-013; requirements.md:388-393 | 기본 `index_dir`=`$PGDATA/cuvs_indexes`(`src/pg_cuvs.c:519`), 제외 훅 없음 → 기본 설정 위반 | PARTIAL/UNVERIFIED | med | ADR-013 주의 노트 (PGDATA 밖 권장) |
...
| ADR-024 3G.2 GCS round-trip | ADR-024; phase-record.md:719 | "자동 검증 불가" 정직하게 disclosed — 과대표기 아님 |

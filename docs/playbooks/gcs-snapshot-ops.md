...
sudo systemctl cat pg-cuvs-server | grep -i snapshot
```

**기대 출력:**
```
ExecStart=... --snapshot-uri gs://<bucket> --cluster-id <cluster>
```
**→ `--snapshot-uri` 없음:** 원인 C(snapshot_uri 미설정) → Step 2로  
**→ 있음:** Step 0으로 넘어가 인증 전제 확인
...
**기대 출력 (정상 업로드):**
```
[INFO] [objstore] sharded snapshot complete db/idx shards=2 ts=... relfilenode=...
```
**→ `upload FAILED ... HTTP 404`:** 원인 D(구버전 PUT→POST 버그) 또는 버킷 미존재  
**→ `corrupt manifest`:** 원인 D(공백 허용 JSON 파서 미적용) → 데몬 바이너리 버전 확인  
**→ `GCS download failed`:** 원인 A(SA 없음) 또는 B(권한 부족)
...
**기대 출력:**
```
gs://<bucket>/db/idx/manifest.json
gs://<bucket>/db/idx/index.tids
gs://<bucket>/db/idx/index.shards
gs://<bucket>/db/idx/index.s000.cagra
gs://<bucket>/db/idx/index.s001.cagra
```
**→ 버킷이 비어 있음:** 업로드가 한 번도 일어나지 않음 → Step 0으로
...
```
**→ 성공:** Step 1으로  
**→ 실패(empty/error/`Request had insufficient authentication scopes`):** SA가 VM에 없음 → `gpu-vm-lifecycle.md`의 set-service-account 먼저 실행

---

### Step 1 — 버킷 준비 + SA grant
...
**기대 출력:**
```
Creating gs://pgcuvs-snap-1234567890/...
Updated IAM policy for bucket [pgcuvs-snap-1234567890].
```
**→ 성공:** Step 2로  
**→ `ALREADY_EXISTS`:** 버킷 이름 충돌 → `BUCKET` 변수 변경 후 재시도  
**→ `PERMISSION_DENIED`:** 로컬 gcloud 계정에 버킷 생성 권한 없음 → 프로젝트 IAM 확인

---

### Step 2 — snapshot 켜고 데몬 실행
...
```bash
# 데몬이 snapshot uri를 파싱했는지 확인
sudo journalctl -u pg-cuvs-server --no-pager | grep -i "snapshot"
```

**기대 출력:**
```
[INFO] pg_cuvs_server: snapshot uri: gs://<bucket>  cluster: <cluster>
```
**→ 성공:** Step 3으로  
**→ `snapshot uri` 메시지 없음:** unit 파일에 `--snapshot-uri` 누락 → `sudo systemctl edit pg-cuvs-server`로 추가 후 재시도

---

### Step 3 — 인덱스 빌드 + 업로드 확인
...
```
**→ 성공:** Step 4로  
**→ 메시지 없음:** 업로드 실패 → `grep -iE "upload|FAILED|ERROR"` 로 오류 확인  
**→ `HTTP 404`:** 원인 D(구버전 바이너리) 또는 버킷 미존재
...
### Step 4 — 복원 테스트 (artifact 삭제 후 warmup)

```bash
# .relfilenode만 남기고 local artifact 삭제 (복원 시나리오 재현)
sudo rm -f /tmp/cuvs_indexes/<db>_<idx>.cagra
sudo rm -f /tmp/cuvs_indexes/<db>_<idx>.tids
sudo rm -f /tmp/cuvs_indexes/<db>_<idx>.shards
sudo rm -f /tmp/cuvs_indexes/<db>_<idx>.s???.cagra
# .relfilenode 는 남겨야 heap 호환 식별 가능
...
**기대 출력:**
```
[INFO] [objstore] sharded download verified OK for db/idx (2 shards)
[INFO] pg_cuvs_server: loaded sharded index db/idx (... vecs, 2 shards)
```
**→ 성공:** 복원 완료 → 검증 체크리스트로  
**→ `HEAP COMPAT MISMATCH`:** 원인 E → 이 노드에서 REINDEX  
**→ `GCS download failed`:** Step 0 인증 재확인

> warmup 완료 전 쿼리는 NOT_FOUND → CPU fallback; 완료 후 GPU path.
...
SELECT count(*) FROM pg_stat_gpu_shards WHERE index_name='<idx>' AND resident;
```
...
- [ ] `sharded snapshot complete` 로그 확인 (업로드 성공)
- [ ] GCS 버킷에 `.tids` / `.shards` / `.sNNN.cagra` / `manifest.json` 모두 존재
- [ ] `sharded download verified OK` 로그 확인 (복원 성공)
- [ ] `pg_stat_gpu_shards` resident 수 == shard_count
